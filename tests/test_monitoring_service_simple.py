...
async def test_service_imports():
    ...
    try:
        ...
        from src.schemas.monitoring_schemas import AlertLevel, AlertRecordSchema, MetricType, MonitoringMetricSchema
...
async def test_cache_manager():
    ...
    try:
        from src.services.monitoring_service import CacheConfig, CacheManager

        config = Cacheget_config()
...
async def test_metrics_storage():
    ...
    try:
        ...
        results = await storage.query_metrics(
            metric_names=["test_metric"], start_time=datetime.now() - timedelta(hours=1), end_time=datetime.now()
        )
...
async def test_monitoring_service():
    ...
    try:
        ...
        results = await service.query_metrics_with_cache(
            metric_names=["service_test_0"], start_time=datetime.now() - timedelta(minutes=5), end_time=datetime.now()
        )
...
async def test_convenience_functions():
    ...
    try:
        from src.services.monitoring_service import CacheConfig, StorageConfig, create_monitoring_service
...
async def main():
    ...
    # 同步测试
    sync_tests = [("文件结构", test_file_structure)]
