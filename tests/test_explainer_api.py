...
class TestExplainerAPI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        np.random.seed(42)
        self.X = pd.DataFrame(
            np.random.rand(100, 5), columns=["feature1", "feature2", "feature3", "feature4", "feature5"]
        )
    ...
    def test_api_explain_request_model(self):
        """测试API请求模型"""
        from src.api.explainer_api import BatchExplainRequest, ExplainRequest, FeatureImportanceRequest
    ...
    def test_api_explain_endpoint(self):
        """测试API解释端点"""
        from src.api.explainer_api import ExplainRequest, explain_prediction
        ...
        # 验证数据一致性
        self.assertEqual(len(response.explanation["feature_names"]), len(response.explanation["shap_values"]))

    def test_api_feature_importance_endpoint(self):
        """测试API特征重要性端点"""
        from src.api.explainer_api import FeatureImportanceRequest, feature_importance
    ...
    def test_api_batch_explain_endpoint(self):
        """测试API批量解释端点"""
        from src.api.explainer_api import BatchExplainRequest, batch_explain
    ...
    def test_api_model_summary_endpoint(self):
        """测试API模型摘要端点"""
        from src.api.explainer_api import ExplainRequest, model_summary
    ...
    def test_api_error_handling(self):
        ...
        from src.api.explainer_api import ExplainRequest, explain_prediction, load_model
...
class TestIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 创建测试数据
        np.random.seed(42)
        self.X = pd.DataFrame(np.random.rand(50, 4), columns=["open", "high", "low", "close"])
