...
try:
    from src.services.export_service import (
        ExportService, ExportConfig, DataExporter, 
        ReportGenerator, PDFReportGenerator, EmailService,
        ScheduledReportService, create_export_service
    )
...
class MockMonitoringService:
    ...
    async def query_metrics_with_cache(self, 
...
class ExportFunctionalityTest:
    ...
    def __init__(self):
        self.logger = logging.getLogger(f"{__name__}.ExportFunctionalityTest")
        ...
        try:
            ...
            if len(original_data) == len(exported_data):
                ...
                for i, (orig, exp) in enumerate(zip(original_data, exported_data)):
                    if (orig['metric_name'] != exp['metric_name'] or 
                        orig['metric_value'] != exp['metric_value']):
