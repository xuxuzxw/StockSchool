...
class TestFactorFeatureStore:
    ...
    def test_store_factor_data(self, mock_feature_store):
        ...
        # 模拟数据库操作
        with patch.object(mock_feature_store, "_record_data_lineage"), patch.object(
            mock_feature_store, "_calculate_quality_metrics"
        ):

            success = mock_feature_store.store_factor_data(
                factor_name="test_factor", version_id="test_factor_v1_abc123", data=test_data
            )
    ...
    def test_get_factor_data(self, mock_feature_store):
        ...
        with patch.object(mock_feature_store, "_get_latest_version", return_value="test_factor_v1_abc123"):
            ...
            result = mock_feature_store.get_factor_data(factor_name="test_factor", ts_codes=["000001.SZ", "000002.SZ"])
    ...
    def test_search_factors(self, mock_feature_store, mock_engine):
        ...
        results = mock_feature_store.search_factors(query="test", factor_type="technical")
...
class TestFeatureStoreAdapter:
    ...
    def mock_adapter(self, mock_engine):
        """模拟适配器"""
        with patch("src.compute.feature_store_adapter.FactorFeatureStore"), patch(
            "src.compute.feature_store_adapter.FactorEngine"
        ), patch("src.compute.feature_store_adapter.TechnicalFactorEngine"), patch(
            "src.compute.feature_store_adapter.FundamentalFactorEngine"
        ), patch(
            "src.compute.feature_store_adapter.SentimentFactorEngine"
        ):
    ...
    def test_calculate_and_store_factor(self, mock_adapter):
        ...
        # 模拟计算结果
        mock_data = pd.DataFrame({"ts_code": ["000001.SZ"], "factor_date": [date(2024, 1, 1)], "test_factor": [10.5]})
    ...
    def test_batch_calculate_and_store(self, mock_adapter):
        ...
        with patch.object(mock_adapter, "calculate_and_store_factor", return_value=(True, "v1")):
            results = mock_adapter.batch_calculate_and_store(
                factor_names=["factor1", "factor2"], ts_codes=["000001.SZ"]
            )
    ...
    def test_compare_factor_versions(self, mock_adapter):
        ...
        comparison = mock_adapter.compare_factor_versions(factor_name="test_factor", version_id1="v1", version_id2="v2")
    ...
    def test_migrate_legacy_data(self, mock_adapter, mock_engine):
        ...
        # 模拟历史数据
        legacy_data = pd.DataFrame({"ts_code": ["000001.SZ"], "factor_date": [date(2024, 1, 1)], "test_factor": [10.5]})
        ...
        with patch("pandas.read_sql", return_value=legacy_data):
            ...
            success = mock_adapter.migrate_legacy_data(
                factor_name="test_factor", source_table="stock_factors_technical"
            )
