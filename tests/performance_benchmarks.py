...
try:
    ...
    from src.schemas.monitoring_schemas import MetricType, MonitoringMetricSchema
...
class PerformanceBenchmark:
    ...
    async def run_benchmark(
        self, test_func: Callable, iterations: int = 100, warmup_iterations: int = 10, **kwargs
    ) -> BenchmarkResult:
        """运行基准测试"""
        # 预热
...
class DatabaseBenchmark(PerformanceBenchmark):
    ...
    async def test_query_performance(self, query_name: str, params: Dict[str, Any]) -> None:
        ...
        try:
            results, metrics = await self.query_optimizer.execute_optimized_query(query_name, params)
            return len(results)
        except Exception as e:
    ...
    async def run_database_benchmarks(self) -> List[BenchmarkResult]:
        ...
        try:
            ...
            result = await self.run_benchmark(
                self.test_query_performance, iterations=50, query_name="get_recent_metrics", params=query_params
            )
            results.append(result)

            # 测试索引创建性能
            result = await self.run_benchmark(
                self.test_index_creation, iterations=1, warmup_iterations=0  # 索引创建只需要测试一次
            )
            results.append(result)
...
class CacheBenchmark(PerformanceBenchmark):
    ...
    def __init__(self, cache_config: Optional[CacheConfig] = None):
        """方法描述"""
        self.cache_config = cache_config or Cacheget_config()
    ...
    async def run_cache_benchmarks(self) -> List[BenchmarkResult]:
        ...
        try:
            ...
            # 测试单个获取性能
            result = await self.run_benchmark(self.test_cache_get, iterations=1000, key="test_key_0")
            results.append(result)

            # 测试批量操作性能
            result = await self.run_benchmark(self.test_batch_operations, iterations=100, data=test_data)
            results.append(result)
...
class WebSocketBenchmark(PerformanceBenchmark):
    ...
    async def test_message_compression(self, message_size: int) -> None:
        ...
        message = WebSocketMessage(type=MessageType.DATA, data=large_data)
    ...
    async def test_batch_messaging(self, batch_size: int) -> None:
        ...
        for i in range(batch_size):
            message = WebSocketMessage(
                type=MessageType.DATA, data={"index": i, "timestamp": datetime.now().isoformat()}
            )
    ...
    async def run_websocket_benchmarks(self) -> List[BenchmarkResult]:
        ...
        try:
            ...
            # 测试消息压缩性能
            result = await self.run_benchmark(
                self.test_message_compression, iterations=100, message_size=5000  # 5KB消息
            )
            results.append(result)

            # 测试批量消息性能
            result = await self.run_benchmark(self.test_batch_messaging, iterations=50, batch_size=20)
            results.append(result)
...
async def run_performance_benchmarks(
    database_url: str = "postgresql://stockschool:stockschool123@localhost:15432/stockschool", save_report: bool = True
) -> Dict[str, Any]:
    """运行性能基准测试"""
