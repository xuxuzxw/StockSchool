...
class TestDataValidator:
    ...
    @staticmethod
    def validate_factor_series(
        series: pd.Series, expected_length: int, value_range: tuple = None, name: str = "Factor"
    ):
        """验证因子序列的基本属性"""
        ...
        if value_range:
            ...
            if not valid_values.empty:
                ...
                assert all(min_val <= val <= max_val for val in valid_values), f"{name} 的值应该在 {value_range} 范围内"

    @staticmethod
    def validate_calculation_results(results: dict, expected_factors: list, data_length: int):
        """验证计算结果的完整性"""
        assert isinstance(results, dict), "计算结果应该是字典类型"

        for factor in expected_factors:
            assert factor in results, f"应该包含因子 {factor}"
            TestDataValidator.validate_factor_series(results[factor], data_length, name=factor)
