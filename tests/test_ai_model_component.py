...
def main():
    ...
    total_tests = (
        len(results["functionality_tests"]) + len(results["visualization_tests"]) + len(results["interaction_tests"])
    )

    avg_accuracy = sum(results["accuracy_tests"].values()) / len(results["accuracy_tests"])

    return (
        functionality_passed + visualization_passed + interaction_passed
    ) / total_tests >= 0.8 and avg_accuracy >= 0.90
