...
class TestDataGenerator:
    ...
    def generate_stock_list(self, count: int = 100, 
                          markets: List[str] = None) -> List[str]:
        """
        生成股票代码列表
    ...
    def generate_trading_calendar(self, start_date: str = "2023-01-01",
                                end_date: str = "2023-12-31") -> List[date]:
        """
        生成交易日历（排除周末和节假日）
    ...
    def generate_price_series(self, length: int, start_price: float = 100.0,
    ...
    def generate_ohlc_data(self, prices: np.ndarray, 
                          intraday_volatility: float = 0.01) -> Dict[str, np.ndarray]:
        """
        基于收盘价生成OHLC数据
    ...
    def generate_volume_data(self, prices: np.ndarray, 
                           base_volume: int = 5000000,
                           price_volume_correlation: float = 0.3) -> np.ndarray:
        """
        生成成交量数据
    ...
    def generate_stock_daily_data(self, stock_codes: List[str],
                                trading_days: List[date],
                                regime: MarketRegime = MarketRegime.SIDEWAYS,
                                start_price_range: Tuple[float, float] = (10.0, 200.0),
                                volatility_range: Tuple[float, float] = (0.015, 0.035)) -> pd.DataFrame:
        """
        生成股票日线数据
        ...
        for stock_code in stock_codes:
            ...
            prices = self.generate_price_series(
                len(trading_days), start_price, regime, volatility
            )
    ...
    def generate_financial_data(self, stock_codes: List[str],
                              report_dates: List[str] = None) -> pd.DataFrame:
        """
        生成财务数据
    ...
    def generate_industry_data(self, stock_codes: List[str]) -> pd.DataFrame:
        ...
        """
        industries = [
            '银行', '保险', '证券', '地产', '建筑', '钢铁', '煤炭', '有色',
            '化工', '石化', '电力', '公用', '交通', '汽车', '机械', '电子',
            '通信', '计算机', '传媒', '医药', '食品', '纺织', '轻工', '商贸',
            '农业', '综合'
        ]
        ...
        for stock_code in stock_codes:
            ...
            data_list.append({
                'ts_code': stock_code,
                'industry': industry,
                'industry_code': f"SW{industries.index(industry)+1:02d}"
            })
        
        return pd.DataFrame(data_list)
    
    def generate_factor_data(self, stock_codes: List[str],
                           trading_days: List[date],
                           factor_names: List[str],
                           factor_correlations: Dict[str, float] = None) -> pd.DataFrame:
        """
        生成因子数据
        ...
        for stock_code in stock_codes:
            ...
            for i, factor_name in enumerate(factor_names):
                if i == 0:
                    ...
                    stock_factors[factor_name] = (
                        base_factor * correlation + 
    ...
    def generate_return_data(self, stock_codes: List[str],
                           trading_days: List[date],
    ...
    def generate_benchmark_data(self, trading_days: List[date],
                              benchmark_name: str = "000300.SH") -> pd.DataFrame:
        """
        生成基准指数数据
    ...
    def create_test_dataset(self, 
                          ...
                          include_factors: bool = True) -> Dict[str, pd.DataFrame]:
        """
        创建完整的测试数据集
        ...
        end_date = (datetime.strptime(start_date, "%Y-%m-%d") + 
                   timedelta(days=int(n_days * 1.4))).strftime("%Y-%m-%d")
        ...
        # 股票日线数据
        dataset['stock_daily'] = self.generate_stock_daily_data(
            stock_codes, trading_days, regime
        )
        ...
        if include_factors:
            factor_names = [
                'sma_5', 'sma_20', 'rsi_14', 'macd', 'pe_ttm', 'pb', 
                'roe', 'money_flow_5', 'attention_score'
            ]
            dataset['factors'] = self.generate_factor_data(
                stock_codes, trading_days, factor_names
            )
