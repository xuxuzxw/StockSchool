...
# 导入重构的组件
from .performance_test_base import BasePerformanceTest, performance_test, PerformanceMetrics
from .test_strategies import (
    SingleFactorTestStrategy, MultipleFactorTestStrategy, 
    ConcurrentTestStrategy, PerformanceTestContext
)
...
class TechnicalFactorPerformanceTest(BasePerformanceTest):
    ...
    def __init__(self, engine, data_generator):
        super().__init__(engine, data_generator)
    ...
    def execute_test(self, test_data: Dict[str, Any], test_params: Dict[str, Any]) -> Any:
        ...
        return context.execute_performance_test(test_data['engine'], {
...
class TestSingleFactorPerformance:
    ...
    def test_sma_calculation_performance(self, performance_tester):
        ...
        logger.info(f"SMA性能指标: 执行时间={metrics.execution_time:.2f}s, "
                   f"吞吐量={metrics.throughput:.2f}记录/秒")
...
class TestMultipleFactorPerformance:
    ...
    def test_multiple_technical_factors_performance(self, performance_tester):
        ...
        logger.info(f"多因子性能指标: 执行时间={metrics.execution_time:.2f}s, "
                   f"总数据量={metrics.data_count}")
...
class TestLargeDatasetPerformance:
    ...
    def test_large_stock_list_performance(self, performance_tester):
        ...
        logger.info(f"大数据集性能: 单股票平均时间={avg_time_per_stock:.3f}s, "
                   f"总吞吐量={metrics.throughput:.2f}记录/秒")
