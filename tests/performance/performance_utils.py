...
class PerformanceProfiler:
    ...
    def take_memory_snapshot(self, label: str = ""):
        ...
        self.memory_snapshots.append({"timestamp": elapsed_time, "memory_mb": current_memory, "label": label})
    ...
    def end_profiling(self) -> Dict[str, Any]:
        ...
        initial_memory = (
            self.memory_snapshots[0]
            if isinstance(self.memory_snapshots[0], (int, float))
            else self.memory_snapshots[0]["memory_mb"]
        )
        memory_usage = final_memory - initial_memory
        peak_memory = max(
            snapshot["memory_mb"] if isinstance(snapshot, dict) else snapshot for snapshot in self.memory_snapshots
        )
...
def benchmark_function(func: Callable, *args, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        profile_data.update({"success": False, "error": str(e), "result": None})
        return profile_data
...
class PerformanceComparator:
    ...
    @staticmethod
    def find_performance_regressions(
        reports: List[PerformanceReport], threshold_percent: float = 10.0
    ) -> List[Dict[str, Any]]:
        """查找性能回归"""
        regressions = []
