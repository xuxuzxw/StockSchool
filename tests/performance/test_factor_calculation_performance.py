...
class TestFactorCalculationPerformance:
    ...
    def large_stock_list(self):
        ...
        return [
            ...
            '600009.SH', '600028.SH', '600030.SH', '600048.SH',
            '000001.SZ', '000002.SZ', '000858.SZ', '000876.SZ'
        ]
    ...
    def test_single_factor_calculation_performance(self, test_engine, large_stock_list):
        ...
        logger.info(f"SMA计算性能: {len(large_stock_list)}只股票, "
    ...
    def test_multiple_factors_calculation_performance(self, test_engine, large_stock_list):
        ...
        factors_to_test = [
            ('sma_5', lambda: technical_engine.calculate_sma(large_stock_list[:10], 
                                                           date.today() - timedelta(days=20), 
                                                           date.today() - timedelta(days=1), 5)),
            ('sma_20', lambda: technical_engine.calculate_sma(large_stock_list[:10], 
                                                            date.today() - timedelta(days=30), 
                                                            date.today() - timedelta(days=1), 20)),
            ('rsi_14', lambda: technical_engine.calculate_rsi(large_stock_list[:10], 
                                                            date.today() - timedelta(days=30), 
                                                            date.today() - timedelta(days=1), 14))
        ]
        ...
        try:
            ...
            for window in [5, 10, 20]:
                try:
                    ...
                    logger.info(f"SMA{window}计算后内存: {current_memory:.1f}MB, "
                               f"数据量: {len(data)}")
            ...
            logger.info(f"内存使用测试: 初始={initial_memory:.1f}MB, "
    ...
    def test_large_dataset_performance(self, test_engine, data_generator):
        ...
        for batch_size in batch_sizes:
            ...
            try:
                ...
                logger.info(f"批量大小{batch_size}: {batch_time:.2f}秒, "
                           f"处理{total_processed}只股票")
    ...
    def test_calculation_accuracy_vs_performance(self, test_engine, data_generator):
        ...
        for precision_name, dtype in precision_tests:
            ...
            try:
                ...
                logger.info(f"{precision_name}精度计算: {calculation_time:.4f}秒, "
