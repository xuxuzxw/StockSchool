...
class PerformanceBenchmark:
    ...
    def validate_metrics(self, execution_time: float, memory_mb: float, data_count: int) -> List[str]:
        """验证性能指标，返回违规项列表"""
        violations = []
...
class PerformanceTestConfig:
    ...
    DEFAULT_CONFIG = {
        "test_data": {"stock_count": 20, "days_range": 90, "initial_price": 100.0, "volatility": 0.02},
        "benchmarks": {
            "single_factor": {
...
# 全局配置实例
performance_config = PerformanceTestConfig(os.path.join(os.path.dirname(__file__), "performance_test_config.yml"))
