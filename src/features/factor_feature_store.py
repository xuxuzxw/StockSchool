...
class FactorVersion:
    ...
    def __init__(self, version_id: str, factor_name: str, version: str, created_at: datetime, metadata: Dict[str, Any]):
...
class FactorFeatureStore:
    ...
    def _init_factor_tables(self):
        ...
        try:
            with self.engine.connect() as conn:
                ...
                # 创建索引
                conn.execute(
                    text("CREATE INDEX IF NOT EXISTS idx_factor_versions_name ON factor_versions(factor_name)")
                )
                conn.execute(
                    text("CREATE INDEX IF NOT EXISTS idx_factor_versions_active ON factor_versions(is_active)")
                )
                conn.execute(
                    text("CREATE INDEX IF NOT EXISTS idx_factor_metadata_type ON factor_metadata(factor_type)")
                )
                conn.execute(
                    text("CREATE INDEX IF NOT EXISTS idx_factor_lineage_factor ON factor_lineage(factor_name)")
                )
                conn.execute(
                    text("CREATE INDEX IF NOT EXISTS idx_factor_quality_date ON factor_quality_metrics(metric_date)")
                )
    ...
    def create_factor_version(
        self, factor_name: str, algorithm_code: str, parameters: Dict[str, Any], metadata: Dict[str, Any] = None
    ) -> str:
        """
        创建因子版本
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                """
                    ...
                    {"factor_name": factor_name, "algorithm_hash": algorithm_hash},
                )
    ...
    def store_factor_data(
        self, factor_name: str, version_id: str, data: pd.DataFrame, partition_date: date = None
    ) -> bool:
        """
        存储因子数据到特征商店
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                """
                    ...
                    {"data_hash": data_hash, "version_id": version_id},
                )

            # 存储到特征商店
            feature_name = f"{factor_name}_{version_id}"
            success = self.feature_store.store_features(
                feature_name=feature_name, data=data_with_version, partition_date=partition_date
            )
    ...
    def get_factor_data(
        ...
        try:
            ...
            query = FactorQuery(
                ...
                end_date=end_date,
                data_source=DataSource.FEATURE_STORE,
            )
            ...
            logger.info(f"获取因子数据 {factor_name} 版本 {version_id}，共 {len(result.data)} 条记录")
            return result.data

        except ImportError:
            # 如果新服务不可用，回退到原实现
            logger.warning("使用旧版实现，建议升级到统一服务")
            return self._legacy_get_factor_data(factor_name, version_id, ts_codes, start_date, end_date)
        except Exception as e:
    ...
    def _legacy_get_factor_data(
        ...
        try:
            ...
            data = self.feature_store.get_features(
                feature_name=feature_name, entity_ids=ts_codes, start_date=start_date, end_date=end_date
            )
    ...
    def get_factor_lineage(self, factor_name: str, version_id: str = None) -> Dict[str, Any]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                return {"factor_name": factor_name, "lineage": lineage_info}
    ...
    def get_quality_metrics(
        self, factor_name: str, version_id: str = None, start_date: date = None, end_date: date = None
    ) -> pd.DataFrame:
        """
        获取因子质量指标
    ...
    def search_factors(
        self, query: str, factor_type: str = None, category: str = None, tags: List[str] = None
    ) -> List[FactorMetadata]:
        """
        搜索因子
    ...
    def _calculate_quality_metrics(
        self, factor_name: str, version_id: str, data: pd.DataFrame, metric_date: date = None
    ):
        """计算数据质量指标"""
        try:
            ...
            if factor_column in data.columns and data[factor_column].notna().sum() > 0:
                ...
                if std_val > 0:
                    ...
                    valid_count = data[
                        (data[factor_column] >= valid_range[0]) & (data[factor_column] <= valid_range[1])
                    ][factor_column].count()
    ...
    def get_storage_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                # 活跃版本数量
                result = conn.execute(
                    text("SELECT COUNT(*) as active_count FROM factor_versions WHERE is_active = true")
                )
