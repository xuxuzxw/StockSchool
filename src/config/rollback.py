...
class ConfigRollbackManager:
    ...
    def create_snapshot(self, snapshot_id: str, description: str = "", 
                       tags: List[str] = None, metadata: Dict[str, Any] = None) -> ConfigSnapshot:
        """创建配置快照"""
        try:
            # 获取当前配置
    ...
    def create_rollback_plan(self, rollback_type: RollbackType, 
                           target_snapshot_id: str = None,
                           target_timestamp: datetime = None,
                           selective_paths: List[str] = None) -> RollbackPlan:
        """创建回滚计划"""
        ...
        if rollback_type == RollbackType.SNAPSHOT:
            ...
            if self.snapshots:
                ...
                changes_to_rollback = [
                    ...
                    if any(change["path"].startswith(path) for path in selective_paths)
                ]
        ...
        return RollbackPlan(
            ...
            target_snapshot=target_snapshot if rollback_type in [RollbackType.SNAPSHOT, RollbackType.TIMESTAMP] else None,
    ...
    def execute_rollback_plan(self, plan: RollbackPlan, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            pre_rollback_snapshot = self.create_snapshot(
                f"pre_rollback_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                "回滚前的配置快照",
                tags=["pre_rollback"]
            )
