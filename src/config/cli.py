...
def cmd_check_compatibility(args):
    ...
    try:
        ...
        if args.migrate and report.migration_required:
            print("🔄 执行自动迁移...")
            migrated_config = checker.migrate_config(
                config_manager._config,
                args.current_version,
                args.target_version
            )
...
示例:
  ...
  %(prog)s info
    ...
    parser_init.add_argument("--environment", default="development", 
    ...
    parser_set.add_argument("--type", choices=["str", "int", "float", "bool", "json"], 
