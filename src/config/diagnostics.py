...
class ConfigDiagnostics:
    ...
    def add_diagnostic_rule(
        self, path_pattern: str, check_function: callable, level: DiagnosticLevel, category: DiagnosticCategory
    ):
        """添加诊断规则"""
        self.diagnostic_rules.append(
            {"path_pattern": path_pattern, "check_function": check_function, "level": level, "category": category}
        )
    ...
    def diagnose_config(self, config: Dict[str, Any], config_path: str = "") -> DiagnosticReport:
        ...
        for rule in self.diagnostic_rules:
            ...
            if pattern == "*":
                ...
                matching_paths = [path for path in flat_config.keys() if fnmatch.fnmatch(path, pattern)]
    ...
    def _check_tushare_token(self, value: Any, path: str) -> Optional[Dict[str, Any]]:
        ...
        if value is True and not os.getenv("TUSHARE_TOKEN"):
            return {
                ...
                "suggestions": ["设置TUSHARE_TOKEN环境变量", "或将data_sync_params.tushare.enabled设置为false"],
            ...
            try:
                ...
                if not torch.cuda.is_available():
                    return {
                        ...
                        "suggestions": ["安装CUDA驱动程序", "或将use_cuda设置为false", "检查PyTorch CUDA版本"],
                    }
            except ImportError:
                return {
                    "message": "启用CUDA但未安装PyTorch",
                    "description": "使用CUDA功能需要安装PyTorch",
                    "suggestions": ["安装PyTorch: pip install torch", "或将use_cuda设置为false"],
                }
        return None
    ...
    def _check_cors_security(self, value: Any, path: str) -> Optional[Dict[str, Any]]:
        ...
        if isinstance(value, list) and "*" in value:
            return {
                ...
                "suggestions": ["指定具体的允许来源", "使用域名而不是通配符", "在开发环境中可以保持当前设置"],
                "auto_fix_available": True,
...
def diagnose_config_file(file_path: str) -> DiagnosticReport:
    ...
    try:
        ...
        return DiagnosticReport(
            ...
            issues_by_category={
                category.value: 1 if category == DiagnosticCategory.SYNTAX else 0 for category in DiagnosticCategory
            },
            issues=[error_issue],
            auto_fixable_count=0,
            health_score=0.0,
...
if __name__ == "__main__":
    ...
    # 测试配置
    test_config = {
        "data_sync_params": {"batch_size": -100, "tushare": {"enabled": True}},  # 错误：负数  # 可能错误：缺少环境变量
        "api_params": {"port": 99999, "cors_origins": ["*"]},  # 错误：端口号超出范围  # 警告：安全问题
        "factor_params": {"macd": {"fast_period": 30, "slow_period": 26}},  # 错误：快周期大于慢周期
    }
