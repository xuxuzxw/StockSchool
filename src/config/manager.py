...
from .error_handling import ConfigError, ConfigLoadError, handle_config_errors
...
class ConfigManager:
    ...
    def __init__(self,
        ...
        # 组件初始化
        self._config_loader = UnifiedConfig()
        self._config_loader._config_dir = str(self.config_dir)  # 传递配置目录
    ...
    def _load_all_configs(self):
        ...
        with self._config_lock:
            # 直接加载配置文件，不依赖UnifiedConfig
            self._config = self._load_config_from_directory()

    def _load_config_from_directory(self) -> Dict[str, Any]:
        """从指定目录加载配置文件"""
        config_data = {}

        try:
            # 加载基础配置
            base_config = self.config_dir / 'base.yml'
            ...
            # 加载环境特定配置
            env_config = self.config_dir / f'{self.environment.value}.yml'
            ...
            # 加载主配置文件作为后备
            main_config = Path('config.yml')
            if main_config.exists() and not config_data:
                import yaml
    ...
    def validate_config(self) -> List[str]:
        """验证配置"""
        result = self._validator.validate_config(self._config)
