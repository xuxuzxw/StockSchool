...
# 页面配置
st.set_page_config(page_title="StockSchool 模型解释器", page_icon="📊", layout="wide", initial_sidebar_state="expanded")
...
def main():
    ...
    # 选择功能
    feature = st.sidebar.radio("选择功能", ["模型信息", "特征重要性", "预测解释", "批量解释", "模型对比"])
            ...
            if data_file is not None:
                ...
                if feature_columns:
                    ...
                    target_column = st.selectbox(
                        "选择目标列（可选）", ["无"] + [col for col in df.columns if col not in feature_columns]
                    )
                    ...
                    method = st.selectbox(
                        ...
                        format_func=lambda x: {"default": "默认重要性", "permutation": "排列重要性", "shap": "SHAP值"}[
                            x
                        ],
                    )
                        ...
                        with st.spinner("正在计算特征重要性..."):
                            try:
                                ...
                                for i, (bar, importance_val) in enumerate(zip(bars, top_features["importance"])):
                                    ax.text(
                                        bar.get_width(), i, f"{importance_val:.4f}", va="center", ha="left", fontsize=8
                                    )
    ...
    if model_file is not None:
        try:
            ...
            if data_file is not None:
                ...
                if feature_columns:
                    # 选择样本索引
                    sample_idx = st.number_input("选择样本索引", min_value=0, max_value=len(df) - 1, value=0)
        ...
        try:
            ...
            if data_file is not None:
                ...
                if feature_columns:
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        start_idx = st.number_input("起始样本索引", min_value=0, max_value=len(df) - 1, value=0)
                    with col2:
                        ...
                        with st.spinner("正在批量解释预测结果..."):
                            try:
                                ...
                                for i, explanation in enumerate(explanations):
                                    with st.expander(
                                        f"样本 {start_idx + i} - 预测值: {explanation.get('prediction', 'N/A'):.4f}"
                                    ):
                                        col1, col2 = st.columns(2)
                                        with col1:
                                            st.write("SHAP值:")
...
def model_comparison_page():
    ...
    model_files = st.file_uploader(
        "上传多个模型文件", type=["pkl", "joblib"], accept_multiple_files=True, key="mc_models"
    )
                    ...
                    if st.button("对比分析", type="primary"):
                        with st.spinner("正在进行模型对比分析..."):
                            try:
                                ...
                                pred_df = pd.DataFrame({"模型": model_names, "预测值": pred_values})
