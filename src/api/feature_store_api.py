...
from src.api.auth import require_factor_admin, require_factor_read, require_factor_write
...
async def register_factor_metadata(
    ...
    try:
        ...
        return APIResponse(success=True, data={"factor_name": factor_name}, message="因子元数据注册成功")
...
async def get_factor_metadata(
    ...
    try:
        if factor_name:
            ...
            return APIResponse(success=True, data=metadata.to_dict(), message="因子元数据获取成功")
        else:
            metadata_list = feature_store.get_factor_metadata()

            return APIResponse(
                success=True,
                data={"factors": [meta.to_dict() for meta in metadata_list], "total_count": len(metadata_list)},
...
async def create_factor_version(
    request: FactorVersionRequest, user=Depends(require_factor_write()), feature_store=Depends(get_factor_feature_store)
):
    """创建因子版本"""
    try:
        ...
        return APIResponse(success=True, data={"version_id": version_id}, message="因子版本创建成功")
...
async def get_factor_versions(
    factor_name: str, user=Depends(require_factor_read()), feature_store=Depends(get_factor_feature_store)
):
    """获取因子版本列表"""
    try:
        ...
        return APIResponse(
            success=True,
            data={"versions": [version.to_dict() for version in versions], "total_count": len(versions)},
...
async def activate_factor_version(
    version_id: str, user=Depends(require_factor_write()), feature_store=Depends(get_factor_feature_store)
):
    """激活因子版本"""
    try:
        ...
        if success:
            return APIResponse(success=True, data={"version_id": version_id}, message="因子版本激活成功")
        else:
...
async def get_factor_lineage(
    ...
    try:
        ...
        return APIResponse(success=True, data=lineage, message="因子血缘获取成功")
...
async def get_factor_quality_metrics(
    ...
    try:
        metrics = feature_store.get_quality_metrics(
            factor_name=factor_name, version_id=version_id, start_date=start_date, end_date=end_date
        )
        ...
        return APIResponse(
            success=True,
            data={"quality_metrics": metrics_dict, "total_count": len(metrics_dict)},
...
async def search_factors(
    request: FactorSearchRequest, user=Depends(require_factor_read()), feature_store=Depends(get_factor_feature_store)
):
    """搜索因子"""
    try:
        factors = feature_store.search_factors(
            query=request.query, factor_type=request.factor_type, category=request.category, tags=request.tags
        )

        return APIResponse(
            success=True,
            data={"factors": [factor.to_dict() for factor in factors], "total_count": len(factors)},
...
async def get_storage_statistics(user=Depends(require_factor_read()), feature_store=Depends(get_factor_feature_store)):
    """获取特征商店存储统计"""
    try:
        stats = feature_store.get_storage_statistics()

        return APIResponse(success=True, data=stats, message="存储统计获取成功")
...
async def health_check():
    ...
    try:
        ...
        return APIResponse(success=False, data={"status": "unhealthy", "error": str(e)}, message="特征商店异常")
