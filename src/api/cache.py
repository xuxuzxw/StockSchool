...
class CacheManager:
    ...
    def _generate_cache_key(self, prefix: str, *args, **kwargs) -> str:
        """生成缓存键"""
        # 创建参数的哈希值
        params_str = json.dumps({"args": args, "kwargs": sorted(kwargs.items())}, sort_keys=True, default=str)
...
def cache_response(prefix: str = "api", ttl: Optional[int] = None, skip_cache: Callable = None):
    """
    API响应缓存装饰器
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            # 生成缓存键
            cache_key = cache_manager._generate_cache_key(f"{prefix}:{func.__name__}", *args, **kwargs)
