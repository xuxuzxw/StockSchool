...
# 角色定义
ROLES = {
    "viewer": {"name": "查看者", "permissions": ["factor:read"]},
    "analyst": {"name": "分析师", "permissions": ["factor:read", "factor:calculate", "system:monitor"]},
    "developer": {
        "name": "开发者",
        "permissions": ["factor:read", "factor:write", "factor:calculate", "factor_management:read", "system:monitor"],
    },
    "admin": {"name": "管理员", "permissions": list(PERMISSIONS.keys())},
}
    ...
    def _init_default_users(self):
        ...
        try:
            with self.engine.connect() as conn:
                # 检查是否已有管理员用户
                result = conn.execute("SELECT COUNT(*) as count FROM users WHERE username = 'admin'")
    ...
    def authenticate_user(self, username: str, password: str) -> Optional[User]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                """,
                    {"user_id": user_data.user_id, "last_login": datetime.now()},
                )
...
def require_permissions(required_permissions: List[str]):
    ...
    def permission_checker(token_data: TokenData = Depends(verify_token)) -> TokenData:
        ...
        if not required_permissions_set.issubset(user_permissions):
            ...
            raise HTTPException(status_code=403, detail=f"权限不足，缺少权限: {', '.join(missing_permissions)}")
...
def require_roles(required_roles: List[str]):
    ...
    def role_checker(token_data: TokenData = Depends(verify_token)) -> TokenData:
        ...
        if not required_roles_set.intersection(user_roles):
            raise HTTPException(status_code=403, detail=f"角色权限不足，需要角色: {', '.join(required_roles)}")
