...
from src.compute.manual_calculation_trigger import CalculationMode, CalculationRequest, ManualCalculationTrigger
...
class FactorService:
    ...
    def query_factors(
        ...
        try:
            # 构建查询参数
            query_params = self._build_query_params(ts_codes, factor_names, factor_types, start_date, end_date)
    ...
    def _build_query_params(
        ...
        """构建查询参数"""
        query_params = {"ts_codes": ts_codes, "start_date": start_date, "end_date": end_date or date.today()}
    ...
    def _execute_factor_query(self, query_params: Dict[str, Any], standardized: bool) -> pd.DataFrame:
        """执行因子查询"""
        if standardized:
    ...
    def _format_factor_response(self, data: pd.DataFrame, query_params: Dict[str, Any]) -> Dict[str, Any]:
        """格式化因子响应数据"""
        result_data = []

        if not data.empty:
            for _, row in data.iterrows():
                factor_values = {col: row[col] for col in data.columns if col not in ["ts_code", "factor_date"]}
                result_data.append(
                    {"ts_code": row["ts_code"], "factor_date": row["factor_date"], "factor_values": factor_values}
                )

        return {"factors": result_data, "total_count": len(result_data), "query_params": query_params}
...
class FactorCalculationService:
    ...
    def _create_calculation_request(self, request_data: Dict[str, Any], mode: CalculationMode) -> CalculationRequest:
        """创建计算请求对象"""
