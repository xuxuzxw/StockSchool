        ...
        def test_training_pipeline():
            ...
            try:
                ...
                pipeline.prepare_training_data(
                    start_date="20240101", end_date="20240131", sample_size=sample_size  # 使用较小的日期范围
                )
    ...
    def _identify_bottlenecks(self, result: Dict) -> List[str]:
        ...
        for func in result["top_functions"]:
            try:
                ...
                if ncalls > 10000:
                    bottlenecks.append(
                        f"函数 '{func['filename_func']}' 调用次数过多 ({ncalls:,} 次），" f"可能存在重复计算"
                    )
        ...
        for func in result["top_functions"]:
            if func["percall_cum"] > 0.1:
                bottlenecks.append(f"函数 '{func['filename_func']}' 单次调用耗时过长 ({func['percall_cum']:.4f} 秒)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--module", choices=["factor_engine", "training_pipeline", "all"], default="all", help="要分析的模块"
    )
    parser.add_argument("--output-dir", type=str, default="performance_reports", help="输出目录")
