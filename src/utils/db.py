...
class DatabaseManager:
    ...
    def _upsert_batch(self, table_name: str, df: pd.DataFrame, conflict_columns: List[str], update_columns: List[str]):
        """执行UPSERT操作
    ...
    def batch_process_stocks(self, stocks: List[str], process_func, batch_size: int = 50, **kwargs) -> Dict[str, Any]:
        """批量处理股票
        ...
        result_stats = {"total": total_stocks, "success": success_count, "error": error_count, "errors": errors}
...
def batch_insert_or_update(
    ...
    """批量插入或更新的便捷函数"""
    return get_db_manager().batch_insert_or_update(table_name, data, conflict_columns, update_columns, batch_size)
...
def get_trading_dates(start_date: Optional[str] = None, end_date: Optional[str] = None) -> List[str]:
    """获取交易日历的便捷函数"""
