...
@click.option('--factor-types', '-t', multiple=True, default=['technical', 'fundamental'], help='要计算的因子类型 (technical, fundamental)。')
@click.option('--start-date', '-sd', help='技术面因子计算的开始日期 (YYYY-MM-DD)。')
@click.option('--end-date', '-ed', help='技术面因子计算的结束日期 (YYYY-MM-DD)。')
@click.option('--init-db', is_flag=True, help='初始化数据库，创建所有需要的表。')
@cli.command()
@click.option('--model-type', '-m', type=click.Choice(['linear_regression', 'random_forest', 'gradient_boosting']), required=True, help='要训练的模型类型。')
...
def calculate_factors(stocks: Optional[List[str]], 
