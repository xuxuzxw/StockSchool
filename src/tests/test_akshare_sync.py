...
class TestAkshareSynchronizer:
    ...
    def test_standardize_news_sentiment(self, syncer):
        ...
        # 准备测试数据
        news_data = [
            {"date": "2024-01-01", "title": "股价上涨利好消息", "content": "公司业绩增长", "source": "test"},
            {"date": "2024-01-01", "title": "股价下跌利空消息", "content": "市场风险增加", "source": "test"},
        ]
    ...
    def test_update_sync_status(self, syncer, mock_engine):
        ...
        syncer._update_sync_status("news_sentiment", "success", records_processed=100, records_failed=5, duration=60)
    ...
    def test_fetch_popularity_ranking_hot(self, mock_hot_rank, syncer):
        ...
        # 模拟akshare返回数据
        mock_df = pd.DataFrame(
            {"代码": ["000001", "000002", "600000"], "热度": [9500, 8800, 8200], "排名变化": [1, -1, 0]}
        )
    ...
    def test_full_sync_success(self, syncer):
        """测试完整同步成功"""
        with patch.object(syncer, "sync_news_sentiment", return_value=True), patch.object(
            syncer, "sync_user_attention", return_value=True
    ...
    def test_full_sync_partial_failure(self, syncer):
        """测试完整同步部分失败"""
        with patch.object(syncer, "sync_news_sentiment", return_value=True), patch.object(
            syncer, "sync_user_attention", return_value=False
        ), patch.object(syncer, "sync_popularity_ranking", return_value=True):
