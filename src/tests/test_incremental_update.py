...
from src.data.incremental_update import IncrementalUpdateManager, SyncPriority, SyncTask
...
class TestIncrementalUpdateManager:
    ...
    def test_get_existing_dates(self, manager, mock_engine):
        ...
        # 模拟数据库查询结果
        mock_dates = [(datetime(2024, 1, 2).date(),), (datetime(2024, 1, 4).date(),)]
        conn.execute.return_value = mock_dates
    ...
    def test_get_missing_dates(self, manager):
        """测试获取缺失日期"""
        with patch.object(
            manager, "_get_trading_dates", return_value=["2024-01-01", "2024-01-02", "2024-01-03"]
    ...
    def test_schedule_incremental_sync(self, manager):
        """测试调度增量同步"""
        with patch.object(manager, "get_missing_dates", return_value=["2024-01-01", "2024-01-02"]), patch.object(
            manager,
            "_get_active_stocks",
            return_value=[
            ...
            created_tasks = manager.schedule_incremental_sync(
                data_sources=["tushare"], data_types=["daily"], days_back=7
            )
    ...
    def test_execute_single_task_success(self, manager):
        ...
        with patch.object(manager, "_call_sync_method", return_value={"records_processed": 1}), patch.object(
            manager, "_update_task_sync_status"
        ):
    ...
    def test_execute_single_task_failure(self, manager):
        ...
        with patch.object(manager, "_call_sync_method", side_effect=Exception("同步失败")), patch.object(
            manager, "_update_task_sync_status"
        ):
    ...
    def test_get_sync_status_summary(self, manager, mock_engine):
        ...
        with patch.object(manager, "_get_trading_dates", return_value=["2024-01-01", "2024-01-02"]), patch.object(
            manager, "_get_existing_dates", return_value=["2024-01-01"]
        ):
...
class TestIncrementalUpdateManagerPerformance:
    ...
    def test_missing_dates_detection_performance(self):
        ...
        # 模拟大量交易日期和已存在日期
        trading_dates = [(datetime(2024, 1, 1) + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(365)]
        existing_dates = [date for i, date in enumerate(trading_dates) if i % 3 != 0]  # 缺失1/3的数据

        with patch.object(manager, "_get_trading_dates", return_value=trading_dates), patch.object(
            manager, "_get_existing_dates", return_value=existing_dates
        ):
    ...
    def test_task_creation_performance(self):
        ...
        for i in range(100):
            manager.create_sync_task(
                "tushare", "daily", f"2024-01-{(i % 30) + 1:02d}", stock_codes[:10], SyncPriority.NORMAL
            )
