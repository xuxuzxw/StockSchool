...
class TimescaleTestDataBuilder:
    ...
    def create_hypertable_data(count=2):
        """创建超表测试数据"""
        return [("stock_daily", 2, 10, True, "default"), ("financial_data", 1, 5, False, "default")][:count]
    ...
    def create_compression_stats(table_name="stock_daily"):
        """创建压缩统计测试数据"""
        return [(table_name, 10, 5, 1000000, 500000, "localhost")]
...
class TestTimescaleOptimizer:
    ...
    SAMPLE_HYPERTABLE_DATA = [("stock_daily", 2, 10, True, "default"), ("financial_data", 1, 5, False, "default")]
    SAMPLE_CHUNK_DATA = [("_timescaledb_internal._hyper_1_1_chunk", "2024-01-01", "2024-02-01", True, "default", None)]
    ...
    def test_optimize_table_success(self, optimizer):
        ...
        with patch.object(optimizer, "create_hypertable", return_value=True), patch.object(
            optimizer, "create_composite_indexes", return_value=True
        ), patch.object(optimizer, "enable_compression", return_value=True), patch.object(
            optimizer, "add_retention_policy", return_value=True
        ):
    ...
    def test_optimize_all_tables_success(self, optimizer):
        """测试优化所有表成功"""
        # Arrange
        with patch.object(optimizer, "setup_timescale_extension", return_value=True), patch.object(
            optimizer, "optimize_table", return_value=True
        ):
    ...
    def test_get_chunk_info_success(self, optimizer, mock_connection):
        ...
        # Arrange
        table_name = "stock_daily"
        mock_data = [("_timescaledb_internal._hyper_1_1_chunk", "2024-01-01", "2024-02-01", True, "default", None)]
        mock_connection.execute.return_value.fetchall.return_value = mock_data
        mock_connection.execute.return_value.keys.return_value = [
            "chunk_name",
            "range_start",
    ...
    def test_get_compression_stats_success(self, optimizer, mock_connection):
        """测试获取压缩统计信息成功"""
        # Arrange
        mock_data = [("stock_daily", 10, 5, 1000000, 500000, "localhost")]
        mock_connection.execute.return_value.fetchall.return_value = mock_data
        mock_connection.execute.return_value.keys.return_value = [
            "hypertable_name",
            "total_chunks",
    ...
    def test_manual_compress_chunks_success(self, optimizer, mock_connection):
        ...
        # Arrange
        table_name = "stock_daily"
        mock_connection.execute.return_value.fetchall.return_value = [("chunk1",), ("chunk2",), ("chunk3",)]
        optimizer.engine.connect.return_value.__enter__.return_value = mock_connection
    ...
    def test_reorder_chunks_success(self, optimizer, mock_connection):
        ...
        # Arrange
        table_name = "stock_daily"
        mock_connection.execute.return_value.fetchall.return_value = [("chunk1",), ("chunk2",)]
        optimizer.engine.connect.return_value.__enter__.return_value = mock_connection
