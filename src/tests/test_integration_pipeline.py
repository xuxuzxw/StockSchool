...
try:
    ...
    from src.compute.technical import calculate_bollinger_bands, calculate_macd, calculate_rsi
    ...
    calculate_macd = lambda df: pd.DataFrame(
        {"macd": [0.0] * len(df), "signal": [0.0] * len(df), "histogram": [0.0] * len(df)}
    )
    calculate_bollinger_bands = lambda df, window: pd.DataFrame(
        {"upper": [0.0] * len(df), "middle": [0.0] * len(df), "lower": [0.0] * len(df)}
    )
...
def setup_test_database():
    ...
    try:
        ...
        success = test_db_manager.setup_test_environment(
            stock_codes=["000001.SZ", "000002.SZ", "600000.SH"], start_date="20240101", end_date="20240131"
        )
...
def test_factor_golden_data_validation():
    ...
    try:
        ...
        assert (
            abs(last_rsi_value - GOLDEN_RSI_VALUE) < tolerance
    ...
    try:
        ...
        if not daily_data.empty:
            ...
            for _, row in daily_data.iterrows():
                ...
                ), f"开盘价应在最高最低价之间: {row['ts_code']} {row['trade_date']}"
                assert (
                    row["low"] <= row["close"] <= row["high"]
                ), f"收盘价应在最高最低价之间: {row['ts_code']} {row['trade_date']}"
