...
from .factor_exceptions import InsufficientDataError, InvalidFactorParameterError
...
class FactorMetadata(BaseFactorModel):
    ...
    def validate(self) -> bool:
        ...
        if self.min_periods < FactorConstants.MIN_WINDOW_SIZE:
            raise InvalidFactorParameterError(
                "min_periods", self.min_periods, f"最小周期不能小于 {FactorConstants.MIN_WINDOW_SIZE}"
            )

        if self.min_periods > FactorConstants.MAX_WINDOW_SIZE:
            raise InvalidFactorParameterError(
                "min_periods", self.min_periods, f"最小周期不能大于 {FactorConstants.MAX_WINDOW_SIZE}"
            )
...
class FactorValue(BaseFactorModel):
    ...
    def validate(self) -> bool:
        ...
        if self.percentile_rank is not None and not (0 <= self.percentile_rank <= 1):
            raise InvalidFactorParameterError("percentile_rank", self.percentile_rank, "分位数排名必须在0-1之间")

        if self.industry_rank is not None and not (0 <= self.industry_rank <= 1):
            raise InvalidFactorParameterError("industry_rank", self.industry_rank, "行业排名必须在0-1之间")
...
class FactorStatistics(BaseFactorModel):
    ...
    def validate(self) -> bool:
        ...
        if self.valid_count < 0 or self.valid_count > self.total_count:
            raise InvalidFactorParameterError("valid_count", self.valid_count, "有效数量必须在0到总数量之间")

        return True

    def calculate_statistics(self, values: List[float], percentiles: List[int] = None) -> None:
        """计算统计信息"""
    ...
    def _calculate_percentiles(self, valid_values: List[float], percentiles: List[int]) -> None:
        """计算分位数"""
...
class FactorMetadataFactory:
    ...
    @staticmethod
    def create_technical_factor(name: str, description: str, category: FactorCategory, **kwargs) -> FactorMetadata:
        """创建技术面因子元数据"""
        return FactorMetadata(
            name=name, description=description, factor_type=FactorType.TECHNICAL, category=category, **kwargs
        )

    @staticmethod
    def create_fundamental_factor(name: str, description: str, category: FactorCategory, **kwargs) -> FactorMetadata:
        """创建基本面因子元数据"""
        return FactorMetadata(
            name=name, description=description, factor_type=FactorType.FUNDAMENTAL, category=category, **kwargs
        )

    @staticmethod
    def create_sentiment_factor(name: str, description: str, category: FactorCategory, **kwargs) -> FactorMetadata:
        """创建情绪面因子元数据"""
        return FactorMetadata(
            name=name, description=description, factor_type=FactorType.SENTIMENT, category=category, **kwargs
        )
