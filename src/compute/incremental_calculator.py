...
from .factor_models import CalculationStatus, FactorResult, FactorType, FactorValue
...
class FactorDependency:
    ...
    def __init__(
        self, factor_name: str, dependency_type: DependencyType, dependencies: List[str], lookback_days: int = 0
    ):
        """
        初始化因子依赖
...
class DependencyManager:
    ...
    def _build_default_dependencies(self):
        ...
        sentiment_dependencies = [
            ...
            FactorDependency(
                "attention_score",
                DependencyType.CALCULATION_DEPENDENCY,
            ...
            FactorDependency(
                "sentiment_strength",
                DependencyType.CALCULATION_DEPENDENCY,
                ["price_momentum_sentiment", "volatility_sentiment"],
                20,
            ),
        ]
...
class IncrementalDataManager:
    ...
    def get_last_calculation_date(self, ts_code: str, factor_name: str) -> Optional[date]:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                result = conn.execute(query, {"ts_code": ts_code, "factor_name": factor_name})
    ...
    def get_missing_dates(self, ts_code: str, factor_name: str, end_date: Optional[date] = None) -> List[date]:
        """获取需要计算的缺失日期"""
        if end_date is None:
            end_date = date.today()
    ...
    def get_incremental_data(self, ts_code: str, missing_dates: List[date], lookback_days: int = 0) -> pd.DataFrame:
        """获取增量计算所需的数据"""
        ...
        try:
            ...
            with self.engine.connect() as conn:
                result = conn.execute(
                    query, {"ts_code": ts_code, "start_date": actual_start_date, "end_date": end_date}
                )
...
class IncrementalStorage:
    ...
    def save_incremental_factors(
        self, ts_code: str, factor_name: str, factor_values: List[FactorValue], replace_existing: bool = False
    ) -> bool:
        """保存增量因子数据"""
        ...
        try:
            ...
            # 如果需要替换现有数据，先删除
            if replace_existing:
                self._delete_existing_factors(ts_code, factor_name, [fv.date for fv in factor_values])

            # 保存数据
            df.to_sql("factor_values", self.engine, if_exists="append", index=False, method="multi")
    ...
    def _delete_existing_factors(self, ts_code: str, factor_name: str, dates: List[date]):
        """删除现有的因子数据"""
        try:
    ...
    def batch_save_factors(
        self, factor_results: Dict[str, Dict[str, List[FactorValue]]], batch_size: int = 1000
    ) -> bool:
        """批量保存因子数据"""
        try:
            all_data = []
            ...
            for i in range(0, len(df), batch_size):
                ...
                batch_df.to_sql("factor_values", self.engine, if_exists="append", index=False, method="multi")

                logger.info(f"保存批次 {i // batch_size + 1}/{total_batches}，" f"包含 {len(batch_df)} 条记录")
    ...
    def update_calculation_log(
        ...
        try:
            ...
            df.to_sql("factor_calculation_log", self.engine, if_exists="append", index=False)
...
class IncrementalFactorCalculator:
    ...
    def calculate_incremental_factors(
        self,
        ts_codes: List[str],
        factor_names: List[str],
                ...
                for factor_name in ordered_factors:
                    try:
                        ...
                        if force_recalculate:
                            # 强制重新计算最近30天
                            missing_dates = self.data_manager.get_trading_dates(end_date - timedelta(days=30), end_date)
                        else:
                            missing_dates = self.data_manager.get_missing_dates(ts_code, factor_name, end_date)
                        ...
                        # 获取增量数据
                        incremental_data = self.data_manager.get_incremental_data(ts_code, missing_dates, max_lookback)
                        ...
                        factor_values = self._calculate_single_factor(factor_name, incremental_data, missing_dates)
        ...
        return {"results": all_results, "statistics": calculation_stats}

    def _calculate_single_factor(
        self, factor_name: str, data: pd.DataFrame, target_dates: List[date]
    ) -> List[FactorValue]:
        """
            ...
            if factor_name == "sma_5":
                ...
                for target_date in target_dates:
                    ...
                    if not date_data.empty and pd.notna(date_data["sma_5"].iloc[0]):
                        factor_values.append(FactorValue(date=target_date, value=float(date_data["sma_5"].iloc[0])))
                ...
                for target_date in target_dates:
                    ...
                    if not date_data.empty:
                        ...
                        if pd.notna(rsi_value):
                            factor_values.append(FactorValue(date=target_date, value=float(rsi_value)))
    ...
    def get_calculation_progress(self, ts_codes: List[str], factor_names: List[str]) -> Dict[str, Any]:
        """获取计算进度"""
        progress_info = {
        ...
        try:
            for ts_code in ts_codes:
                stock_progress = {"total_factors": len(factor_names), "completed_factors": 0, "factor_status": {}}
                ...
                stock_progress["progress_rate"] = stock_progress["completed_factors"] / stock_progress["total_factors"]
