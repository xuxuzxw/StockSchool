...
from .factor_models import FactorResult, FactorType, CalculationStatus
...
class WorkerTaskResult:
    ...
    def __init__(self, ts_code: str, success: bool, 
                 result: FactorResult = None, error: str = None):
...
class IndividualWorkerExecutor(BaseWorkerExecutor):
    ...
    def execute_individual(self, tasks: List[Tuple], **kwargs) -> List[FactorResult]:
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            # 提交任务
            future_to_task = {
                executor.submit(execute_single_stock_calculation, task): task
                for task in tasks
            }
...
class BatchWorkerExecutor(BaseWorkerExecutor):
    ...
    def execute_batch(self, tasks: List[Tuple], batch_size: int = None, **kwargs) -> List[FactorResult]:
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            # 提交批量任务
            future_to_batch = {
                executor.submit(execute_batch_calculation, batch): batch
            ...
            for future in as_completed(future_to_batch):
                ...
                try:
                    batch_results = self._handle_future_result(
                        future, f"批次 {len(batch)} 个任务"
                    )
