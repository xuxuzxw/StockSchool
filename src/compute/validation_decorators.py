    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            for df in dataframes:
                _validate_single_dataframe(
                    df, required_columns, min_length, allow_empty, func.__name__
                )
...
def _validate_single_dataframe(df: pd.DataFrame, 
...
def validate_factor_return_data(factor_col: str = 'factor_value',
    ...
    def decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            # 假设前两个参数是factor_data和return_data
            if len(args) >= 2:
                ...
                if len(valid_samples) < min_samples:
                    raise InsufficientDataError(
                        min_samples, len(valid_samples), func.__name__
                    )
