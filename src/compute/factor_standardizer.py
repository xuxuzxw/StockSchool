...
from .factor_models import FactorType, FactorCategory
...
class FactorStandardizer:
    ...
    def zscore_standardize(self, factor_data: pd.Series, 
                          outlier_treatment: OutlierTreatment = OutlierTreatment.CLIP,
                          clip_quantiles: Tuple[float, float] = (0.01, 0.99)) -> pd.Series:
        """
        Z-score标准化
    ...
    def quantile_standardize(self, factor_data: pd.Series,
                           n_quantiles: int = 100) -> pd.Series:
        """
        分位数标准化（转换为均匀分布）
    ...
    def rank_standardize(self, factor_data: pd.Series,
    ...
    def minmax_standardize(self, factor_data: pd.Series,
    ...
    def _handle_outliers(self, factor_data: pd.Series,
                        treatment: OutlierTreatment,
                        clip_quantiles: Tuple[float, float]) -> pd.Series:
        """
        处理异常值
    ...
    def standardize(self, factor_data: pd.Series,
...
class IndustryFactorScorer:
    ...
    def industry_standardize(self, factor_data: pd.DataFrame,
                           ts_code_col: str = 'ts_code',
                           factor_col: str = 'factor_value',
                           method: StandardizationMethod = StandardizationMethod.ZSCORE) -> pd.DataFrame:
        """
        按行业进行因子标准化
        ...
        if industry_mapping.empty:
            ...
            factor_data['standardized_value'] = self.standardizer.standardize(
                factor_data[factor_col], method=method
            )
            return factor_data
        ...
        for industry, group in merged_data.groupby('industry'):
            if len(group) < 2:
                ...
                # 行业内标准化
                group_standardized = self.standardizer.standardize(
                    group[factor_col], method=method
                )
            
            standardized_values.append(pd.DataFrame({
        ...
        final_result = factor_data.merge(
            result[[ts_code_col, 'standardized_value']], 
            on=ts_code_col, 
            how='left'
        )
    ...
    def industry_rank(self, factor_data: pd.DataFrame,
                     ts_code_col: str = 'ts_code',
                     factor_col: str = 'factor_value',
        ...
        if industry_mapping.empty:
            ...
            factor_data['industry_rank'] = factor_data[factor_col].rank(
                ascending=ascending, na_option='keep'
            )
        ...
        for industry, group in merged_data.groupby('industry'):
            ...
            # 行业内排名
            group_copy['industry_rank'] = group[factor_col].rank(
                ascending=ascending, na_option='keep'
            )
        ...
        # 选择需要的列
        result_cols = [ts_code_col, 'industry', 'industry_rank', 
                      'industry_rank_pct', 'industry_stock_count']
    ...
    def industry_neutralize(self, factor_data: pd.DataFrame,
                          ts_code_col: str = 'ts_code',
                          factor_col: str = 'factor_value') -> pd.DataFrame:
        """
        行业中性化处理
        ...
        merged_data['neutralized_value'] = merged_data.apply(
            lambda row: row[factor_col] - industry_means.get(row['industry'], 0)
            if pd.notna(row[factor_col]) else np.nan,
            axis=1
        )
        
        # 合并回原数据
        final_result = factor_data.merge(
            merged_data[[ts_code_col, 'neutralized_value']], 
            on=ts_code_col, 
            how='left'
        )
...
actorQuantileCalculator:
    ...
    def calculate_market_quantiles(self, factor_data: pd.Series,
    ...
    def calculate_industry_quantiles(self, factor_data: pd.DataFrame,
                                   ts_code_col: str = 'ts_code',
                                   factor_col: str = 'factor_value',
        ...
        if industry_mapping.empty:
            ...
            market_quantiles = self.calculate_market_quantiles(
                factor_data[factor_col], n_quantiles
            )
        ...
        for industry, group in merged_data.groupby('industry'):
            if len(group) < n_quantiles:
                # 行业内股票数量太少，使用简单排名
                group_copy = group.copy()
                group_copy['industry_quantile'] = group[factor_col].rank(
                    method='first', na_option='keep'
                )
                group_copy['industry_quantile_rank'] = (
                    group_copy['industry_quantile'] / len(group)
                )
            else:
                # 计算行业内分位数
                industry_quantiles = self.calculate_market_quantiles(
                    group[factor_col], n_quantiles
                )
        ...
        # 合并回原数据
        final_result = factor_data.merge(
            result[[ts_code_col, 'industry_quantile', 'industry_quantile_rank']], 
            on=ts_code_col, 
            how='left'
        )
    ...
    def update_dynamic_quantiles(self, factor_data: pd.Series,
                               historical_quantiles: Optional[pd.DataFrame] = None,
                               decay_factor: float = 0.95) -> pd.DataFrame:
        """
        动态更新分位数
        ...
        for _, current_stat in current_quantiles.quantile_stats.iterrows():
            ...
            # 查找历史对应分位数
            hist_stat = historical_quantiles.quantile_stats[
                historical_quantiles.quantile_stats['quantile'] == quantile
            ]
            ...
            if not hist_stat.empty:
                ...
                updated_stat = {
                    ...
                    'min_value': (decay_factor * hist_stat['min_value'] + 
                                (1 - decay_factor) * current_stat['min_value']),
                    'max_value': (decay_factor * hist_stat['max_value'] + 
                                (1 - decay_factor) * current_stat['max_value']),
                    'mean_value': (decay_factor * hist_stat['mean_value'] + 
                                 (1 - decay_factor) * current_stat['mean_value']),
                    'std_value': (decay_factor * hist_stat['std_value'] + 
                                (1 - decay_factor) * current_stat['std_value'])
                }
            else:
...
class FactorComposer:
    ...
    def equal_weight_compose(self, factor_data: pd.DataFrame,
    ...
    def weighted_compose(self, factor_data: pd.DataFrame,
    ...
    def ic_weighted_compose(self, factor_data: pd.DataFrame,
    ...
    def pca_compose(self, factor_data: pd.DataFrame,
                   factor_cols: List[str],
        ...
        composite_factor = pd.Series(
            ...
            data=composite_values
        ).reindex(factor_data.index)
        
        return composite_factor
    
    def validate_composition(self, original_factors: pd.DataFrame,
                           composite_factor: pd.Series,
                           factor_cols: List[str]) -> Dict[str, float]:
        """
        验证因子合成的有效性
...
class FactorScoreManager:
    ...
    def save_factor_scores(self, scores_data: pd.DataFrame,
                          score_date: date,
                          factor_name: str,
                          score_type: str = 'standardized') -> bool:
        """
        保存因子评分到数据库
    ...
    def load_factor_scores(self, factor_name: str,
                          start_date: Optional[date] = None,
                          end_date: Optional[date] = None,
                          score_type: str = 'standardized') -> pd.DataFrame:
        """
        加载历史因子评分
    ...
    def analyze_score_stability(self, factor_name: str,
        ...
        try:
            ...
            scores_data = self.load_factor_scores(
            ...
            if stock_stability:
                ...
                # 时间序列稳定性
                daily_stats = scores_data.groupby('score_date').agg({
                    'standardized_value': ['mean', 'std', 'count']
                }).round(4)
