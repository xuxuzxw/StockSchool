...
from .effectiveness_config import (
    EffectivenessAnalysisConfig, AnalysisType, StatisticalConstants,
    get_analysis_config, DEFAULT_EFFECTIVENESS_CONFIG
)
from .statistical_strategies import StatisticalAnalyzer, ICCalculationStrategy, LayeredBacktestStrategy
from .effectiveness_analysis_template import EffectivenessAnalysisTemplate
from .validation_decorators import (
    validate_dataframe_input, validate_factor_return_data, 
    handle_analysis_errors, log_analysis_performance
)
...
class FactorEffectivenessAnalyzer:
    ...
    def _register_strategies(self):
        """注册统计分析策略"""
        # IC计算策略
        ic_strategy = ICCalculationStrategy(
            min_samples=self.config.ic_config.min_samples
        )
    ...
    def get_return_data(self, ts_codes: List[str], 
                       start_date: date, end_date: date,
                       return_period: ReturnPeriod = ReturnPeriod.DAILY) -> pd.DataFrame:
        """
        获取股票收益率数据
    ...
    def calculate_ic(self, factor_data: pd.DataFrame, return_data: pd.DataFrame,
                    ...
                    ts_code_col: str = 'ts_code') -> pd.DataFrame:
        """
        计算信息系数(IC)
        ...
        try:
            ...
            for trade_date, group in merged_data.groupby(date_col):
                ...
                # 计算Pearson相关系数作为IC
                ic_pearson, p_value_pearson = stats.pearsonr(
                    valid_data[factor_col], valid_data[return_col]
                )
                
                # 计算Spearman相关系数作为Rank IC
                ic_spearman, p_value_spearman = stats.spearmanr(
                    valid_data[factor_col], valid_data[return_col]
                )
    ...
    def calculate_ir(self, ic_data: pd.DataFrame, 
    ...
    def factor_layered_backtest(self, factor_data: pd.DataFrame, 
        ...
        try:
            ...
            for trade_date, group in merged_data.groupby(date_col):
                ...
                # 计算各层收益率
                layer_stats = valid_data.groupby('factor_quantile')[return_col].agg([
    ...
    def analyze_layered_performance(self, layered_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # 按分层计算平均表现
            layer_performance = layered_data.groupby('factor_quantile').agg({
                'mean': ['mean', 'std'],
                'count': 'mean'
            }).round(6)
            ...
            if len(layered_data) > 1:
                ...
                layer_groups = [
                    group['mean'].values 
                    for _, group in layered_data.groupby('factor_quantile')
                ]
    ...
    def calculate_factor_decay(self, factor_data: pd.DataFrame,
                             return_data: pd.DataFrame,
                             max_periods: int = 20,
                             factor_col: str = 'factor_value',
                             return_col: str = 'return_rate',
            ...
            for period in range(1, max_periods + 1):
                ...
                if shifted_returns:
                    ...
                    ic_period = self.calculate_ic(
                        factor_data, return_shifted_all,
                        factor_col, return_col, date_col, ts_code_col
                    )
    ...
    def analyze_factor_decay(self, decay_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'abs_avg_ic_pearson' in decay_data.columns:
                optimal_period_pearson = decay_data.loc[
                    decay_data['abs_avg_ic_pearson'].idxmax(), 'period'
                ]
            ...
            if 'abs_avg_ic_spearman' in decay_data.columns:
                optimal_period_spearman = decay_data.loc[
                    decay_data['abs_avg_ic_spearman'].idxmax(), 'period'
                ]
            ...
            if len(decay_data) > 1:
                ...
                # 找到第一个低于一半IC的期数
                half_life_periods = decay_data[
                    decay_data['abs_avg_ic_pearson'] <= half_ic
                ]
...
s FactorCorrelationAnalyzer:
    ...
    def calculate_factor_correlation_matrix(self, factor_data: pd.DataFrame,
    ...
    def analyze_correlation_changes(self, factor_data: pd.DataFrame,
                                  factor_cols: List[str],
                                  date_col: str = 'trade_date',
                                  window: int = 60,
                                  method: str = 'pearson') -> pd.DataFrame:
        ...
        try:
            ...
            for i in range(window, len(factor_data_sorted)):
                ...
                # 计算该窗口的相关性矩阵
                corr_matrix = self.calculate_factor_correlation_matrix(
                    window_data, factor_cols, method
                )
    ...
    def identify_high_correlation_factors(self, correlation_matrix: pd.DataFrame,
    ...
    def calculate_factor_independence(self, factor_data: pd.DataFrame,
...
class FactorEffectivenessReporter:
    ...
    def generate_comprehensive_report(self, factor_data: pd.DataFrame,
                                    return_data: pd.DataFrame,
                                    factor_name: str,
                                    factor_col: str = 'factor_value') -> Dict[str, Any]:
        """
        生成因子有效性综合报告
    ...
    def _generate_data_summary(self, factor_data: pd.DataFrame, 
                             return_data: pd.DataFrame) -> Dict[str, Any]:
        """生成数据概览"""
        summary = {}
        ...
        try:
            ...
            if not factor_data.empty:
                summary['factor_data'] = {
                    ...
                    'date_range': {
                        'start': factor_data['trade_date'].min().isoformat() if 'trade_date' in factor_data.columns else None,
                        'end': factor_data['trade_date'].max().isoformat() if 'trade_date' in factor_data.columns else None
                    },
            ...
            if not return_data.empty:
                summary['return_data'] = {
                    ...
                    'return_volatility': return_data['return_rate'].std() if 'return_rate' in return_data.columns else 0
                }
    ...
    def generate_factor_recommendation(self, ranked_factors: List[Dict[str, Any]],
