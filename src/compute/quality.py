...
def validate_price_data(df: pd.DataFrame) -> Dict[str, List[str]]:
    ...
    Returns:
        包含各种错误类型的字典
    """
    errors = {"negative_prices": [], "invalid_ohlc": [], "extreme_changes": [], "zero_volume": []}
    ...
    for col in price_columns:
        if col in df.columns:
            ...
            if negative_mask.any():
                errors["negative_prices"].extend(df[negative_mask].index.tolist())
    ...
    if all(col in df.columns for col in price_columns):
        ...
        if invalid_ohlc.any():
            errors["invalid_ohlc"].extend(df[invalid_ohlc].index.tolist())
    ...
    if "close" in df.columns and "pre_close" in df.columns:
        ...
        if extreme_change.any():
            errors["extreme_changes"].extend(df[extreme_change].index.tolist())

    # 检查零成交量
    if "vol" in df.columns:
        zero_vol = df["vol"] == 0
        if zero_vol.any():
            errors["zero_volume"].extend(df[zero_vol].index.tolist())
...
def calculate_data_quality_score(df: pd.DataFrame) -> Dict[str, float]:
    ...
    if "trade_date" in df.columns:
        ...
        scores["timeliness"] = (
            normal_intervals.sum() / len(date_diffs.dropna()) if len(date_diffs.dropna()) > 0 else 1.0
        )
    else:
...
class DataQualityMonitor:
    ...
    def check_stock_data_quality(self, ts_code: str, days: int = 30) -> Dict:
        ...
        with self.engine.connect() as conn:
            df = pd.read_sql(text(query), conn, params={"ts_code": ts_code, "days": days})

        if df.empty:
            return {"ts_code": ts_code, "status": "NO_DATA", "message": "无数据", "scores": {}}
    ...
    def batch_quality_check(self, stock_codes: List[str] = None, sample_size: int = None) -> List[Dict]:
        ...
        for ts_code in stock_codes:
            try:
                ...
                results.append({"ts_code": ts_code, "status": "ERROR", "message": str(e), "scores": {}})
