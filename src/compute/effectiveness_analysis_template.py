...
from .performance_decorators import memory_monitor_decorator, timing_decorator
...
class EffectivenessAnalysisTemplate(ABC):
    ...
    def analyze(self, factor_data: pd.DataFrame, return_data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        """
        模板方法：执行完整的有效性分析流程
    ...
    def _validate_input_data(self, factor_data: pd.DataFrame, return_data: pd.DataFrame):
        ...
        missing_return_cols = [col for col in required_return_cols if col not in return_data.columns]
        ...
        if len(factor_data) < min_samples:
            self._validation_errors.append(f"因子数据量不足，需要至少{min_samples}条，实际{len(factor_data)}条")
    ...
    def _merge_data(self, factor_data: pd.DataFrame, return_data: pd.DataFrame) -> pd.DataFrame:
        ...
        merged_data = pd.merge(factor_data, return_data, on=merge_keys, how="inner", suffixes=("_factor", "_return"))
