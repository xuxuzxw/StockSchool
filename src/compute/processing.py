...
class FactorProcessor:
    ...
    def load_factor_data(
        self, factor_names: List[str], start_date: str, end_date: str, stock_list: Optional[List[str]] = None
    ) -> pd.DataFrame:
        """加载因子数据
        ...
        params = {"factor_names": factor_names, "start_date": start_date, "end_date": end_date}
        ...
        try:
            ...
            factor_df = df.pivot_table(
                index=["ts_code", "trade_date"], columns="factor_name", values="factor_value", aggfunc="first"
            ).reset_index()
    ...
    def winsorize_factors(
        ...
        for factor in factor_columns:
            ...
            for date in df["trade_date"].unique():
                ...
                # 应用去极值
                df.loc[date_mask, factor] = np.clip(df.loc[date_mask, factor], lower_bound, upper_bound)

        logger.info("因子去极值处理完成")
        return df

    def standardize_factors(
        self, factor_df: pd.DataFrame, factor_columns: List[str], method: str = "zscore"
    ) -> pd.DataFrame:
        """因子标准化处理
    ...
    def neutralize_factors(
        self, factor_df: pd.DataFrame, factor_columns: List[str], neutralize_factors: List[str] = None
    ) -> pd.DataFrame:
        """因子中性化处理
        ...
        if "market_cap" in neutralize_factors:
            market_cap_data = self._load_market_cap_data(
                df["ts_code"].unique().tolist(), df["trade_date"].min(), df["trade_date"].max()
            )
        ...
        cols_to_drop = [
            ...
            if col.startswith("industry_") or col in ["industry", "market_cap", "log_market_cap"]
        ]
    ...
    def _load_market_cap_data(self, stock_list: List[str], start_date: str, end_date: str) -> pd.DataFrame:
        """加载市值数据
        ...
        params = {"stock_list": stock_list, "start_date": start_date, "end_date": end_date}
    ...
    def calculate_factor_correlation(
        self, factor_df: pd.DataFrame, factor_columns: List[str], method: str = "pearson"
    ) -> pd.DataFrame:
        """计算因子相关性矩阵
    ...
    def process_factors_pipeline(
        ...
        if standardize:
            ...
            factor_df = self.standardize_factors(
                factor_df, factor_names, method=standardize_config.get("method", "zscore")
            )
        ...
        if neutralize:
            ...
            factor_df = self.neutralize_factors(
                factor_df, factor_names, neutralize_factors=neutralize_config.get("factors", ["industry", "market_cap"])
            )
    ...
    def _save_processed_factors(self, factor_df: pd.DataFrame, factor_names: List[str]) -> None:
        """保存处理后的因子数据
        ...
        try:
            with self.engine.connect() as conn:
                ...
                # 插入新数据
                processed_df.to_sql("factor_library", conn, if_exists="append", index=False, method="multi")
    ...
    def get_factor_statistics(self, factor_df: pd.DataFrame, factor_columns: List[str]) -> Dict:
        """获取因子统计信息
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action", choices=["process", "correlation", "statistics"], default="process", help="执行动作"
    )
    ...
    if args.action == "process":
        ...
        # 计算因子相关性
        factor_df = processor.load_factor_data(args.factors, args.start_date, args.end_date, args.stocks)
