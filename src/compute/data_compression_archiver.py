...
class CompressionEngine:
    ...
    def compress_data(self, data: Any, compression_level: CompressionLevel = None) -> bytes:
        ...
        try:
            ...
            # 压缩数据
            compressed_data = gzip.compress(serialized_data, compresslevel=compression_level.value)
...
class ArchiveManager:
    ...
    def _ensure_directories(self):
        """确保目录存在"""
        directories = [self.config.archive_path, self.config.backup_path]
    ...
    def archive_factor_data(self, ts_code: str, factor_name: str, cutoff_date: date) -> bool:
        """归档单个因子数据"""
        try:
            # 获取要归档的数据
            query = text(
            ...
            with self.engine.connect() as conn:
                result = conn.execute(
                    query, {"ts_code": ts_code, "factor_name": factor_name, "cutoff_date": cutoff_date}
                )
            ...
            # 记录归档信息
            self._record_archive_info(
                ts_code, factor_name, cutoff_date, archive_filepath, len(data), len(compressed_data)
            )
            ...
            with self.engine.connect() as conn:
                result = conn.execute(
                    delete_query, {"ts_code": ts_code, "factor_name": factor_name, "cutoff_date": cutoff_date}
                )

                deleted_count = result.rowcount

            logger.info(
                f"成功归档 {ts_code} 的 {factor_name} 因子，"
    ...
    def _record_archive_info(
        ...
        try:
            ...
            df.to_sql("factor_archive_log", self.engine, if_exists="append", index=False)

        except Exception as e:
            logger.error(f"记录归档信息失败: {e}")

    def restore_archived_data(self, ts_code: str, factor_name: str, archive_date: date) -> bool:
        """恢复归档数据"""
        try:
            # 查找归档文件
            ...
            # 恢复到数据库
            data.to_sql("factor_values", self.engine, if_exists="append", index=False)

            logger.info(f"成功恢复 {ts_code} 的 {factor_name} 因子数据，" f"恢复 {len(data)} 条记录")
    ...
    def batch_archive(self, cutoff_date: date, max_workers: int = None) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for item in archivable_data:
                future = executor.submit(self.archive_factor_data, item["ts_code"], item["factor_name"], cutoff_date)
...
class DataCleanupManager:
    ...
    def cleanup_old_data(self, cleanup_date: date) -> Dict[str, int]:
        """清理旧数据"""
        cleanup_stats = {"factor_values_deleted": 0, "archive_files_deleted": 0, "log_entries_deleted": 0}
    ...
    def get_storage_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 归档文件统计
            archive_path = Path(self.config.archive_path)
            archive_stats = {"total_files": 0, "total_size_mb": 0}
...
class BackupManager:
    ...
    def create_backup(self, backup_name: str = None) -> bool:
        ...
        try:
            ...
            backup_df.to_sql("backup_log", self.engine, if_exists="append", index=False)
...
class DataCompressionArchiver:
    ...
    def __init__(self, engine, config: DataCompressionConfig = None):
        """初始化数据压缩归档器"""
        self.engine = engine
        self.config = config or DataCompressionget_config()
    ...
    def run_daily_maintenance(self) -> Dict[str, Any]:
        ...
        maintenance_results = {"archive_stats": {}, "cleanup_stats": {}, "backup_success": False, "storage_stats": {}}
