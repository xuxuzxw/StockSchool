...
from .data_validation_mixin import DataValidationMixin, DataValidationError
...
class AbstractFactorCalculator(ABC, DataValidationMixin):
    ...
    def _postprocess_and_validate(self, results: Dict[str, pd.Series], 
    ...
    def _validate_calculation_results(self, results: Dict[str, pd.Series], 
        ...
        for factor_name, factor_series in results.items():
            if len(factor_series) != data_length:
                raise DataValidationError(
                    f"因子 {factor_name} 的结果长度 {len(factor_series)} "
                    f"与输入数据长度 {data_length} 不匹配"
                )
