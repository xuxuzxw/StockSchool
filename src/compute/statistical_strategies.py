...
class ICCalculationStrategy(StatisticalStrategy):
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        ...
        return (
            not data.empty and 
            all(col in data.columns for col in required_cols) and
            len(data.dropna(subset=required_cols)) >= self.min_samples
        )
    ...
    def calculate(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        ic_pearson, p_value_pearson = stats.pearsonr(
            valid_data['factor_value'], valid_data['return_rate']
        )
...
class LayeredBacktestStrategy(StatisticalStrategy):
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        ...
        return (
            not data.empty and 
            all(col in data.columns for col in required_cols) and
            len(data.dropna(subset=required_cols)) >= self.n_layers * self.min_stocks_per_layer
        )
    ...
    def calculate(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        # 计算各层统计信息
        layer_stats = valid_data.groupby('factor_quantile')['return_rate'].agg([
