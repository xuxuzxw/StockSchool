...
class ResourceManager:
    ...
    def check_resource_availability(self) -> Dict[str, bool]:
        ...
        return {
            ...
            "overall_available": (cpu_percent < self.max_cpu_percent and memory_percent < self.max_memory_percent),
        }
    ...
    def can_run_task(self, task_config: TaskConfig) -> bool:
        ...
        with self.resource_lock:
            ...
            return projected_cpu < self.max_cpu_percent and projected_memory_mb < available_memory_mb
...
class TaskScheduler:
    ...
    def submit_task(self, task_id: str, parameters: Dict[str, Any] = None) -> str:
        ...
        execution = TaskExecution(execution_id=execution_id, task_id=task_id, status=TaskStatus.PENDING)
    ...
    def _execute_task(self, task_config: TaskConfig, execution: TaskExecution) -> TaskExecution:
        ...
        try:
            ...
            if execution.retry_count < task_config.max_retries:
                ...
                logger.warning(
                    f"任务执行失败，准备重试: {task_config.task_name}, "
    ...
    def _save_execution_record(self, execution: TaskExecution):
        ...
        try:
            ...
            df.to_sql("task_execution_log", self.engine, if_exists="append", index=False)
