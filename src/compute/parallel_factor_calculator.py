...
from .factor_models import CalculationStatus, FactorCategory, FactorResult, FactorType
...
class ResourceMonitor:
    ...
    def check_resource_availability(self, min_memory_mb: float = 1000, max_cpu_percent: float = 80) -> Dict[str, bool]:
        """检查资源可用性"""
        ...
        return {
            ...
            "overall_available": (memory_info["available_mb"] > min_memory_mb and cpu_usage < max_cpu_percent),
        }
...
class TaskLoadBalancer:
    ...
    def calculate_optimal_workers(self, task_count: int, task_complexity: str = "medium") -> int:
        """计算最优工作进程数"""
        # 基础工作进程数
    ...
    def balance_workload(self, tasks: List[Any], worker_capacities: Optional[List[float]] = None) -> List[List[Any]]:
        """根据工作进程能力平衡工作负载"""
...
class ParallelFactorCalculator:
    ...
    def __init__(self, engine, config: Optional["ParallelCalculationConfig"] = None):
        ...
        # 组件初始化
        self.connection_manager = EngineConnectionManager(str(engine.url), self.config.database_pool_size)
    ...
    def calculate_factors_parallel(
        ...
        # 准备计算参数
        calculation_params = {"start_date": start_date, "end_date": end_date, "factor_names": factor_names}

        engine_config = {"database_url": self.database_url}
            ...
            for factor_type in factor_types:
                ...
                if batch_mode:
                    ...
                    results = self._calculate_batch_parallel(
                        ts_codes, factor_type, engine_config, calculation_params, max_workers
                    )
                else:
                    # 单股票模式：每个股票单独并行处理
    ...
    def _calculate_batch_parallel(
        ...
        if max_workers is None:
            max_workers = self.load_balancer.calculate_optimal_workers(len(ts_codes), task_complexity="medium")
            ...
            for chunk in stock_chunks:
                if chunk:  # 确保块不为空
                    future = executor.submit(
                        calculate_batch_factors_worker, (chunk, factor_type, engine_config, calculation_params)
                    )
    ...
    def _calculate_individual_parallel(
        self,
        ts_codes: List[str],
        factor_type: str,
        engine_config: Dict[str, Any],
        ...
        # 计算最优工作进程数
        if max_workers is None:
            max_workers = self.load_balancer.calculate_optimal_workers(len(ts_codes), task_complexity="low")
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for ts_code in ts_codes:
                future = executor.submit(
                    calculate_stock_factors_worker, (ts_code, factor_type, engine_config, calculation_params)
                )
    ...
    def _update_performance_stats(self, results: List[FactorResult]):
        ...
        if self.performance_stats["total_calculations"] > 0:
            self.performance_stats["average_execution_time"] = (
                self.performance_stats["total_execution_time"] / self.performance_stats["total_calculations"]
            )
    ...
    def get_optimal_batch_size(self, total_stocks: int, available_memory_mb: float = None) -> int:
        """计算最优批次大小"""
    ...
    def calculate_with_resource_monitoring(
        self, ts_codes: List[str], factor_types: List[str], **kwargs
    ) -> Dict[str, List[FactorResult]]:
        """带资源监控的因子计算"""
        # 检查初始资源状态
