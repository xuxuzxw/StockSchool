...
class FactorConfigManager:
    ...
    def __init__(self, config_dir: Path = None):
        self.config_dir = config_dir or Path("config/factors")
    ...
    def get_calculation_order(self) -> List[str]:
        ...
        sorted_factors = sorted(
            self.configs.items(),
            key=lambda x: x[1].calculation_order
        )
    ...
    def check_for_updates(self) -> List[str]:
        ...
        for config_file in self.config_dir.glob("*.yml"):
            ...
            for factor_name in self.configs:
                if (factor_name in self._last_modified and 
                    file_mtime > self._last_modified[factor_name]):
