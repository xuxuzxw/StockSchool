...
class ProfitabilityFactorCalculator(BaseFactorCalculator):
    ...
    def calculate_roe(self, financial_data: pd.DataFrame) -> pd.Series:
        """计算净资产收益率"""
        # ROE = 净利润 / 股东权益
        roe = financial_data['net_profit'] / financial_data['total_hldr_eqy_exc_min_int']
        
        # 转换为百分比并过滤异常值
        roe = roe * 100
        roe = roe.where((roe > -100) & (roe < 100))
    ...
    def calculate(self, financial_data: pd.DataFrame, **kwargs) -> Dict[str, pd.Series]:
        ...
        return results
class 
GrowthFactorCalculator(BaseFactorCalculator):
    """成长性因子计算器"""
    ...
    def calculate_revenue_growth(self, financial_data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        for ts_code in financial_data['ts_code'].unique():
            ...
            # 将结果添加到对应的索引位置
            for idx, (yoy, qoq, cagr) in zip(stock_data.index, 
                                           zip(revenue_yoy, revenue_qoq, revenue_cagr_3y)):
        ...
        return {
            ...
            'revenue_cagr_3y': revenue_cagr_3y_series.where((revenue_cagr_3y_series > -100) & (revenue_cagr_3y_series < 1000))
        }
    ...
    def calculate_profit_growth(self, financial_data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        for ts_code in financial_data['ts_code'].unique():
            ...
            # 将结果添加到对应的索引位置
            for idx, (yoy, qoq, cagr) in zip(stock_data.index, 
                                           zip(profit_yoy, profit_qoq, profit_cagr_3y)):
                if ts_code not in results:
                    results[ts_code] = {'yoy': {}, 'qoq': {}, 'cagr_3y': {}}
        ...
        return {
            ...
            'net_profit_cagr_3y': profit_cagr_3y_series.where((profit_cagr_3y_series > -1000) & (profit_cagr_3y_series < 1000))
        }
...
class FundamentalFactorEngine(BaseFactorEngine):
    ...
    def get_required_data(self, 
                         ...
                         end_date: Optional[date] = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """获取计算所需的财务数据和市场数据"""
        # 获取财务数据
        financial_query = """
    ...
    def calculate_factors(self, 
        ...
        try:
            ...
            logger.info(f"股票 {ts_code} 基本面因子计算完成，"
