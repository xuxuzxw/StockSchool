...
# 尝试导入新的统一训练服务
try:
    from .training_service import ModelTrainingService, TrainingConfig, TrainingResult

    _HAS_UNIFIED_SERVICE = True
except ImportError:
    _HAS_UNIFIED_SERVICE = False
    logger.warning("无法导入统一训练服务，使用旧版实现")
...
class ModelTrainingPipeline:
    ...
    def __init__(self):
        ...
        # 初始化统一训练服务
        if _HAS_UNIFIED_SERVICE:
            self.training_service = ModelTrainingService(str(self.models_dir))
        else:
    ...
    def run_pipeline(
        ...
        """运行完整的模型训练流水线（使用统一训练服务）"""
        if not self.training_service:
            raise RuntimeError("统一训练服务不可用，无法运行流水线")
        ...
        try:
            # 1. 准备数据
            features, targets = self.prepare_training_data(factor_names, start_date, end_date, target_period)
            if features.empty or targets.empty:
                raise ValueError("数据准备失败，无法继续训练")

            # 2. 使用统一训练服务训练模型
            from src.ai.training_service import TrainingConfig

            config = TrainingConfig(model_type=model_type, model_params=hyperparams or {})

            result = self.training_service.train_model(features, targets, model_type, config)

            self.training_time = result.training_time
            metrics = result.metrics["test"]

            # 3. 保存模型
            self.model_path, self.version = self.save_model(result, model_type)
    ...
    def prepare_training_data(
        ...
        # 1. 加载因子数据
        factor_df = self.factor_processor.load_factor_data(factor_names, start_date, end_date, stock_list)
        ...
        target_df = self._calculate_target_returns(
            factor_df["ts_code"].unique().tolist(), start_date, end_date, target_period
        )

        # 3. 合并数据
        training_data = factor_df.merge(target_df, on=["ts_code", "trade_date"], how="inner")
    ...
    def evaluate_model(self, model: Any, features: pd.DataFrame, targets: pd.DataFrame) -> Dict[str, float]:
        """评估模型性能（已废弃，使用training_service代替）"""
        logger.warning("evaluate_model方法已废弃，请使用training_service")
        if self.training_service:
            # 使用新的统一服务进行评估
            from src.ai.training_service import TrainingConfig

            config = TrainingConfig("lightgbm")  # 默认配置
            result = self.training_service.train_model(features, targets, "lightgbm", config)
            return result.metrics["test"]
        else:
            raise RuntimeError("统一训练服务不可用")

    def save_model(self, model: Any, model_type: str, metrics: Dict[str, float]) -> Tuple[str, str]:
        """保存模型和元数据"""
    ...
    def _calculate_target_returns(
        self, stock_list: List[str], start_date: str, end_date: str, target_period: int
    ) -> pd.DataFrame:
        """计算目标收益率
        ...
        try:
            ...
            # 只保留原始日期范围内的数据
            target_df = target_df[(target_df["trade_date"] >= start_date) & (target_df["trade_date"] <= end_date)]
    ...
    def _clean_training_data(self, training_data: pd.DataFrame, factor_names: List[str]) -> pd.DataFrame:
        """清洗训练数据
    ...
    def train_model(
        ...
        Returns:
            训练结果字典
        """
        logger.warning("train_model方法已废弃，请使用training_service")
        if self.training_service:
            from src.ai.training_service import TrainingConfig

            config = TrainingConfig(
                model_type=model_type, model_params=model_params or {}, cv_folds=cv_folds, test_size=test_size
            )
            result = self.training_service.train_model(features, targets, model_type, config)
            return {
                "model": result.model,
    ...
    def _get_default_model_params(self, model_type: str) -> Dict:
        """获取默认模型参数（已废弃）"""
        logger.warning("_get_default_model_params方法已废弃，使用training_service代替")
        return {}

    def _calculate_metrics(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        """计算评估指标（已废弃）"""
        logger.warning("_calculate_metrics方法已废弃，使用training_service代替")
        return {}

    def _get_feature_importance(self, model, feature_columns: List[str]) -> Dict[str, float]:
        """获取特征重要性（已废弃）"""
        logger.warning("_get_feature_importance方法已废弃，使用training_service代替")
        return {}

    def save_model(self, training_result: Dict[str, Any], model_name: str, version: str = None) -> str:
        """保存模型
    ...
    def load_model(self, model_path: str) -> Dict[str, Any]:
        ...
        result = {"model": model, "scaler": scaler, **metadata}
            ...
            for config in model_configs:
                ...
                try:
                    ...
                    result = self.train_model(
                        features=features, targets=targets, model_type=model_type, model_params=model_params
                    )
                    ...
                    # 保存模型
                    if save_models:
                        model_path = self.save_model(result, f"{model_name}_{model_type}")
    ...
    if result:
        ...
        if result["best_model"]:
            ...
            if best["feature_importance"]:
                importance_sorted = sorted(best["feature_importance"].items(), key=lambda x: x[1], reverse=True)[:10]
                print("\n特征重要性 (Top 10):")
