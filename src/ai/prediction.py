...
class StockPredictor:
    ...
    def __init__(self, model_path: str = None):
        ...
        # 预测配置
        self.prediction_config = config.get(
            "prediction",
            {
    ...
    def generate_prediction_report(self, predictions: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if not stock_info.empty:
                industry_stats = (
                    report_data.groupby("industry")
                    .agg({"prediction_score": ["count", "mean"], "prediction_label": "sum"})
                    .round(4)
                )
    ...
    def run_daily_prediction(self, trade_date: str = None) -> Dict:
        ...
        try:
            ...
            return {"success": True, "predictions": predictions.to_dict("records"), "report": report}
    ...
    if args.output:
        ...
        if "report" in result and "top_predictions" in result["report"]:
            for i, pred in enumerate(result["report"]["top_predictions"][:10], 1):
                print(
                    f"{i:2d}. {pred.get('name', pred['ts_code'])} ({pred['ts_code']}) - "
