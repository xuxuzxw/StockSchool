...
class BacktestEngine:
    ...
    def run_backtest(self, config: BacktestConfig) -> Optional[BacktestResult]:
        ...
        try:
                        ...
                        for stock_code, signal_data in buy_signals.items():
                            if len(positions) < config.max_positions:
                                trade = self._execute_buy(
                                    config, portfolio, signal_data, current_date
                                )
                                if trade:
                                    trades.append(trade)
                                    portfolio['cash'] -= trade.amount + trade.commission
            ...
            result = self._calculate_backtest_result(
                config, start_date, end_date, trading_dates,
                daily_portfolio_values, positions_history, trades, benchmark_data
            )
    ...
    def _validate_config(self, config: BacktestConfig) -> bool:
        ...
        try:
            # 检查模型是否存在
            model_data = self.model_manager.load_model_for_prediction(
    ...
    def _calculate_realtime_scores(self, config: BacktestConfig, current_date: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # 使用评分引擎计算评分
            scores = self.scoring_engine.calculate_stock_scores(
                df, config.model_name, config.model_version
            )
            ...
            # 添加股票信息
            scores = scores.merge(
                df[['stock_code', 'stock_name', 'industry']].drop_duplicates(),
                on='stock_code',
                how='left'
            )
    ...
    def _generate_signals(self, 
                        ...
                        current_date: str) -> Tuple[Dict[str, Dict], Dict[str, str]]:
        """生成交易信号
        ...
        try:
            ...
            if config.selection_method == 'top_score':
                ...
                ].head(config.max_positions - len(positions) + len(sell_signals))
            else:
                # 选择评分超过阈值的股票
                candidates = stock_scores[
                    (~stock_scores['stock_code'].isin(current_positions)) &
            ...
            if config.industry_neutral:
                candidates = self._apply_industry_neutral(
                    candidates, positions, config.max_weight_per_industry
                )
    ...
    def _apply_industry_neutral(self, 
                              candidates: pd.DataFrame,
                              positions: Dict[str, Position],
                              max_weight_per_industry: float) -> pd.DataFrame:
        """应用行业中性约束"""
        try:
            # 计算当前行业权重
            industry_weights = {}
    ...
    def _execute_buy(self, 
                   ...
                   trade_date: str) -> Optional[Trade]:
        """执行买入交易"""
        try:
            stock_code = list(signal_data.keys())[0] if isinstance(signal_data, dict) and len(signal_data) == 1 else signal_data.get('stock_code')
            if not stock_code:
    ...
    def _calculate_backtest_result(self, 
                                 ...
                                 positions_history: List[List[Position]],
                                 trades: List[Trade],
                                 benchmark_data: Optional[pd.DataFrame]) -> Optional[BacktestResult]:
        """计算回测结果"""
        try:
            # 转换为DataFrame
            ...
            for sell_trade in sell_trades:
                ...
                for trade in trades:
                    if (trade.trade_type == 'buy' and 
                        trade.stock_code == sell_trade.stock_code and 
                ...
                if buy_trade:
                    ...
                    # 计算持仓天数
                    holding_days = (datetime.strptime(sell_trade.trade_date, '%Y-%m-%d') - 
                                  datetime.strptime(buy_trade.trade_date, '%Y-%m-%d')).days
            ...
            turnover_rate = total_trade_amount / avg_portfolio_value / (trading_days_count / 252) if avg_portfolio_value > 0 else 0
    ...
    def _create_performance_chart(self, 
                                ...
                                benchmark_data: Optional[pd.DataFrame]) -> Optional[str]:
        """创建业绩图表"""
        try:
            ...
            if benchmark_data is not None and not benchmark_data.empty:
                ...
                ax.plot(benchmark_data.index, benchmark_cumret, 
    ...
    def _save_backtest_result(self, result: BacktestResult):
        ...
        try:
            ...
            with self.engine.connect() as conn:
                result_row = conn.execute(text(insert_sql), {
                    ...
                    'portfolio_value_history': result.portfolio_value_history.to_json(orient='records', date_format='iso'),
    ...
    def _create_comparison_chart(self, results: List[BacktestResult]) -> Optional[str]:
        ...
        try:
            ...
            for result in results:
                ...
                # 简单标准化到0-1
                max_vals = [max(r.total_return for r in results),
