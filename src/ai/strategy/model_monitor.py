...
class ModelMonitor:
    ...
    def monitor_model_performance(
        ...
        try:
            ...
            self._log_monitor_task(
                "performance_monitoring", model_name, start_time, datetime.now(), "failed", 0, errors={"error": str(e)}
            )
            return None
    ...
    def detect_data_drift(self, model_name: str, feature_data: pd.DataFrame) -> List[DataDriftMetrics]:
        ...
        try:
            ...
            logger.info(
                f"模型{model_name}数据漂移检测完成，检测到{len([d for d in drift_metrics if d.is_significant])}个显著漂移"
            )
            ...
            self._log_monitor_task(
                "data_drift_detection", model_name, start_time, datetime.now(), "failed", 0, errors={"error": str(e)}
            )
            return []
    ...
    def _get_recent_predictions(self, model_name: str, model_version: str = None, days: int = 1) -> List[Dict]:
        ...
        try:
            ...
            params = {"model_name": model_name, "start_date": start_date, "end_date": end_date}
    ...
    def _check_performance_thresholds(self, metrics: ModelPerformanceMetrics):
        ...
        try:
            ...
            if metrics.accuracy < self.performance_thresholds["accuracy"]:
                alerts.append(
                    self._create_performance_alert(
                        metrics.model_name, "accuracy", self.performance_thresholds["accuracy"], metrics.accuracy
                    )
                )
            ...
            if metrics.precision < self.performance_thresholds["precision"]:
                alerts.append(
                    self._create_performance_alert(
                        metrics.model_name, "precision", self.performance_thresholds["precision"], metrics.precision
                    )
                )
            ...
            if metrics.recall < self.performance_thresholds["recall"]:
                alerts.append(
                    self._create_performance_alert(
                        metrics.model_name, "recall", self.performance_thresholds["recall"], metrics.recall
                    )
                )
            ...
            if metrics.f1_score < self.performance_thresholds["f1_score"]:
                alerts.append(
                    self._create_performance_alert(
                        metrics.model_name, "f1_score", self.performance_thresholds["f1_score"], metrics.f1_score
                    )
                )
            ...
            if metrics.error_rate > self.performance_thresholds["error_rate"]:
                alerts.append(
                    self._create_performance_alert(
                        metrics.model_name, "error_rate", self.performance_thresholds["error_rate"], metrics.error_rate
                    )
                )
    ...
    def _create_performance_alert(
        self, model_name: str, metric_name: str, threshold: float, actual_value: float
    ) -> Optional[ModelAlert]:
        """创建性能告警"""
        try:
    ...
    def _generate_monitoring_charts(
        self, model_name: str, performance_data: List[Dict], drift_data: List[Dict]
    ) -> Dict[str, str]:
        """生成监控图表"""
        try:
            charts = {}
    ...
    def _create_drift_heatmap(self, drift_data: List[Dict]) -> str:
        ...
        try:
            ...
            # 创建透视表
            pivot_table = df.pivot_table(values="drift_score", index="feature_name", columns="date", aggfunc="mean")
    ...
    def _calculate_summary_stats(
        self, performance_data: List[Dict], drift_data: List[Dict], alert_data: List[Dict]
    ) -> Dict[str, Any]:
        """计算汇总统计"""
        try:
            summary = {"performance_summary": {}, "drift_summary": {}, "alert_summary": {}}
            ...
            if performance_data:
                ...
                summary["performance_summary"] = {
                    ...
                    "performance_trend": (
                        "improving" if perf_df["accuracy"].iloc[-1] > perf_df["accuracy"].iloc[0] else "declining"
                    ),
                }
    ...
    def _get_performance_data(self, model_name: str, start_date: datetime, end_date: datetime) -> List[Dict]:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                result = conn.execute(
                    text(query_sql), {"model_name": model_name, "start_date": start_date, "end_date": end_date}
                )
                rows = result.fetchall()
                ...
                data.append(
                    {
                        ...
                        "precision": float(row[2]) if row[2] else 0,
                        "recall": float(row[3]) if row[3] else 0,
                        "f1_score": float(row[4]) if row[4] else 0,
                        "error_rate": float(row[5]) if row[5] else 0,
                        "prediction_count": row[6],
                        "avg_prediction_time": float(row[7]) if row[7] else 0,
                    }
                )
                ...
                data.append(
                    {
                        ...
                        "p_value": float(row[5]) if row[5] else 0,
                    }
                )
        ...
        try:
            ...
            if metrics.accuracy < self.performance_thresholds["accuracy"]:
                performance_issues.append(
                    f"accuracy: {metrics.accuracy:.4f} < {self.performance_thresholds['accuracy']}"
                )

            if metrics.f1_score < self.performance_thresholds["f1_score"]:
                performance_issues.append(
                    f"f1_score: {metrics.f1_score:.4f} < {self.performance_thresholds['f1_score']}"
                )

            if metrics.error_rate > self.performance_thresholds["error_rate"]:
                performance_issues.append(
                    f"error_rate: {metrics.error_rate:.4f} > {self.performance_thresholds['error_rate']}"
                )
            ...
            if performance_issues:
                trigger = RetrainingTrigger(
                    ...
                    trigger_condition={"issues": performance_issues, "metrics": asdict(metrics)},
                    is_triggered=True,
    ...
    def _start_retraining_job(self, trigger: RetrainingTrigger):
        ...
        try:
            ...
            with self.engine.connect() as conn:
                conn.execute(text(update_sql), {"job_id": job_id, "trigger_id": trigger.trigger_id})
                conn.commit()
    ...
    def _get_recent_significant_drifts(self, model_name: str, days: int = 1) -> List[Dict]:
        ...
        try:
            ...
            drifts = []
            for row in rows:
                drifts.append({"feature_name": row[0], "drift_score": float(row[1]), "drift_type": row[2]})
                ...
                models.append(
                    {
                        ...
                        "health_status": (
                            "healthy" if alert_count == 0 else "warning" if alert_count < 3 else "critical"
                        ),
                    }
                )
    ...
    def _get_performance_trends(self, model_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if model_name:
                where_clause = "WHERE model_name = :model_name"
                params["model_name"] = model_name
                where_clause += " AND date >= CURRENT_DATE - INTERVAL '7 days'"
            else:
                where_clause = "WHERE date >= CURRENT_DATE - INTERVAL '7 days'"
            ...
            trends = {"dates": [], "accuracy": [], "f1_score": [], "error_rate": []}
    ...
    def _get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "system_status": (
                    "healthy" if overall_success_rate >= 95 else "warning" if overall_success_rate >= 80 else "critical"
                ),
            }
