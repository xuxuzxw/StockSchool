...
class SystemOptimizer:
    ...
    def execute_optimization_task(self, task: OptimizationTask) -> Dict[str, Any]:
        ...
        try:
            ...
            # 更新任务状态为完成
            self._update_task_status(task.task_id, "completed", end_time=datetime.now(), progress=100.0, result=result)
            ...
            # 更新任务状态为失败
            self._update_task_status(task.task_id, "failed", end_time=datetime.now(), error_message=str(e))
    ...
    def get_performance_trends(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                result = conn.execute(text(query_sql), {"start_date": start_date, "end_date": end_date})
    ...
    def _get_active_alert(self, alert_type: str, metric_name: str) -> Optional[str]:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                result = conn.execute(text(query_sql), {"alert_type": alert_type, "metric_name": metric_name})
                row = result.fetchone()
    ...
    def _save_performance_baseline(
        self,
        metric_name: str,
        ...
        try:
            ...
            with self.engine.connect() as conn:
                result = conn.execute(
                    text(query_sql), {"metric_name": metric_name, "measurement_period": measurement_period}
                )
    ...
    def _calculate_health_score(self, metrics: SystemMetrics) -> float:
        ...
        # 加权平均
        health_score = (
            cpu_score * 0.2 + memory_score * 0.2 + response_score * 0.25 + cache_score * 0.15 + error_score * 0.2
        )
