...
class Stage3Manager:
    ...
    async def _check_module_health(self, module_name: str) -> bool:
        ...
        try:
            if module_name == 'model_monitor':
                # 检查模型监控模块
                dashboard_data = await self.model_monitor.get_monitoring_dashboard_data()
                return dashboard_data is not None
            
            elif module_name == 'system_optimizer':
                # 检查系统优化模块
                health_report = await self.system_optimizer.get_system_health_report()
    ...
    async def _get_active_alerts_count(self) -> int:
        """获取活跃告警数量"""
        try:
            alerts = await self.model_monitor.get_alerts(status='active', limit=1000)
            return len(alerts)
        except Exception:
            return 0
    ...
    async def _get_test_executions_count(self) -> int:
        ...
        try:
            ...
            results = await self.test_framework.get_test_results(
    ...
    async def _check_model_monitor_details(self) -> Dict[str, Any]:
        ...
        try:
            dashboard_data = await self.model_monitor.get_monitoring_dashboard_data()
            alerts = await self.model_monitor.get_alerts(limit=10)
    ...
    async def _check_test_framework_details(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'status': 'healthy',
                'recent_test_results': len(results),
                'last_check': datetime.now()
            }
        except Exception as e:
            return {
