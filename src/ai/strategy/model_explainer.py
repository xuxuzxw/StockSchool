...
class ModelExplainer:
    ...
    def _get_training_data_sample(self, model_name: str, model_version: str, sample_size: int = 1000) -> Optional[pd.DataFrame]:
        """获取训练数据样本用于解释器初始化"""
        try:
    ...
    def _create_shap_explainer(self, model_data: Dict[str, Any], background_data: pd.DataFrame) -> Optional[Any]:
        ...
        try:
            ...
            if 'tree' in model_type or 'forest' in model_type or 'gbm' in model_type or 'xgb' in model_type or 'lgb' in model_type:
                # 树模型使用TreeExplainer
    ...
    def _get_or_create_explainer(self, 
                               model_name: str, 
                               model_version: str, 
                               explanation_type: str) -> Optional[Any]:
        """获取或创建解释器（带缓存）"""
    ...
    def explain_prediction_shap(self, 
                              ...
                              prediction_date: str = None) -> Optional[ExplanationResult]:
        """使用SHAP解释单个预测
        ...
        try:
            ...
            stock_data = pd.read_sql(query_sql, self.engine, params={
            ...
            # 生成可视化
            visualization_data = self._create_shap_visualization(
                shap_values, feature_columns, base_value, stock_code
            )
    ...
    def explain_prediction_lime(self, 
                              ...
                              num_features: int = 10) -> Optional[ExplanationResult]:
        """使用LIME解释单个预测
        ...
        try:
            ...
            # 生成可视化
            visualization_data = self._create_lime_visualization(
                explanation, stock_code
            )
    ...
    def explain_prediction_permutation(self, 
                                     stock_code: str,
                                     model_name: str,
                                     model_version: str = None,
                                     prediction_date: str = None) -> Optional[ExplanationResult]:
        ...
        try:
            ...
            # 生成可视化
            visualization_data = self._create_permutation_visualization(
                feature_importances, stock_code
            )
    ...
    def create_global_explanation(self, 
                                model_name: str,
                                model_version: str = None,
                                explanation_type: str = 'shap',
                                sample_size: int = 1000) -> Optional[GlobalExplanation]:
        """创建全局模型解释
        ...
        try:
            ...
            # 生成可视化
            visualization_data = self._create_global_visualization(
                feature_importances, model_name, explanation_type
            )
    ...
    def _create_shap_visualization(self, 
                                 shap_values: np.ndarray,
                                 feature_names: List[str],
                                 base_value: float,
                                 stock_code: str) -> Optional[str]:
        """创建SHAP可视化"""
        try:
    ...
    def _create_permutation_visualization(self, 
                                        feature_importances: Dict[str, float],
                                        stock_code: str) -> Optional[str]:
        """创建排列重要性可视化"""
        try:
    ...
    def _create_global_visualization(self, 
                                   feature_importances: Dict[str, float],
                                   model_name: str,
    ...
    def get_global_explanation(self, 
                             model_name: str,
                             model_version: str = None,
                             explanation_type: str = 'shap') -> Optional[GlobalExplanation]:
        """获取全局解释
