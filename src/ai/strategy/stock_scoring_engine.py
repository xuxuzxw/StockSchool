...
class StockScoringEngine:
    ...
    def get_factor_data(self, 
        ...
        try:
            ...
            logger.info(f"获取因子数据成功: {len(df)}只股票, {len(factor_names)}个因子")
            return df
    ...
    def calculate_stock_scores(self,
                             ...
                             normalization_method: str = 'zscore') -> List[StockScore]:
        """计算股票评分
        ...
        try:
            # 获取因子权重
            factor_weights = self.factor_weight_engine.get_factor_weights(
                model_name, model_version, calculation_date
            )
            ...
            # 标准化因子数据
            normalized_data = self.normalize_factors(
                factor_data, factor_names, normalization_method, industry_neutral
            )
    ...
    def get_stock_scores(self,
                        ...
                        industry_code: Optional[str] = None) -> List[StockScore]:
        """获取股票评分
    ...
    def get_top_stocks(self,
        ...
        try:
            if industry_neutral:
                # 按行业均匀选择
                return self._get_industry_neutral_stocks(
                    model_name, model_version, calculation_date, top_n, min_score
                )
            else:
                # 直接按评分排序选择
                stock_scores = self.get_stock_scores(
                    model_name, model_version, calculation_date, top_n
                )
    ...
    def _get_industry_neutral_stocks(self,
                                   model_name: str,
                                   model_version: str,
                                   calculation_date: str,
                                   total_count: int,
    ...
    def update_daily_scores(self,
                          ...
                          model_version: str,
                          calculation_date: str,
                          stock_list: Optional[List[str]] = None) -> bool:
        """更新每日股票评分
