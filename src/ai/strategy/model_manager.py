...
try:
    from src.ai.training_service import ModelTrainingService, TrainingConfig

    _USE_UNIFIED_SERVICE = True
except ImportError:
    _USE_UNIFIED_SERVICE = False
...
class AIModelManager(ModelTrainingPipeline):
    ...
    def create_model_version(
        self, name: str, model_type: str, description: str = "", created_by: str = "system"
    ) -> str:
        """创建新的模型版本
    ...
    def train_and_save_model(
        ...
        try:
            # 使用统一训练服务（如果可用）
            if _USE_UNIFIED_SERVICE and len(model_configs) == 1:
                try:
                    from src.ai.training_service import TrainingConfig

                    config = TrainingConfig(
                        model_type=model_configs[0]["type"],
                        model_params=model_configs[0].get("params", {}),
                        cv_folds=5,
                        test_size=0.2,
                        random_state=42,
                    )
                    ...
                    # 准备训练数据
                    features_df, targets_df = self.prepare_training_data(
                    ...
                    training_result = service.train_model(features_df, targets_df, config)
                    ...
                    result = {
                        "best_model": {
                            ...
                            "model_params": training_result["hyperparameters"],
                        ...
                        "test_date_range": {"start": start_date, "end": end_date},
                        "training_samples": len(features_df),
                        "test_samples": len(features_df),
                    }

                    logger.info(f"使用统一训练服务完成模型训练: {model_name} v{version}")
                except Exception as e:
                    logger.warning(f"统一训练服务失败，使用回退实现: {str(e)}")
            ...
            if result and result.get("best_model"):
                ...
                return {"model_name": model_name, "version": version, "status": "success", "result": result}
            else:
                # 标记为失败
                self._mark_model_failed(model_name, version, "训练失败")
                return {"model_name": model_name, "version": version, "status": "failed", "error": "训练失败"}

        except Exception as e:
            logger.error(f"模型训练失败: {e}")
            self._mark_model_failed(model_name, version, str(e))
            return {"model_name": model_name, "version": version, "status": "failed", "error": str(e)}

    def _update_model_record(
        self, model_name: str, version: str, training_result: Dict[str, Any], start_date: str, end_date: str
    ):
        """更新模型记录"""
    ...
    def _mark_model_failed(self, model_name: str, version: str, error_message: str):
        ...
        try:
            with self.engine.connect() as conn:
                conn.execute(text(update_sql), {"name": model_name, "version": version, "error": error_message})
                conn.commit()
    ...
    def deploy_model(self, model_name: str, version: str) -> bool:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                result = conn.execute(text(activate_sql), {"name": model_name, "version": version})
                conn.commit()
    ...
    def delete_model_version(self, model_name: str, version: str) -> bool:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                result = conn.execute(text(delete_sql), {"name": model_name, "version": version})
                conn.commit()
    ...
    def _legacy_train_model(
        ...
        Returns:
            训练结果字典
        """
        logger.info(f"开始训练 {model_type} 模型（回退实现）")
