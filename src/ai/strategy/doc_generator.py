...
from sqlalchemy import create_engine, text, MetaData, Table, Column, Integer, String, Text, DateTime, Boolean, JSON
...
class DocumentGenerator:
    ...
    def __init__(self, database_url: str = None, project_root: str = None):
        ...
        self.jinja_env = Environment(
            loader=FileSystemLoader(str(self.project_root / 'templates')),
            autoescape=True
        )
    ...
    def _create_tables(self):
        ...
        try:
            ...
            document_templates = Table(
                'document_templates', self.metadata,
                Column('id', Integer, primary_key=True, autoincrement=True),
            ...
            generated_documents = Table(
                'generated_documents', self.metadata,
                Column('id', Integer, primary_key=True, autoincrement=True),
            ...
            document_changes = Table(
                'document_changes', self.metadata,
                Column('id', Integer, primary_key=True, autoincrement=True),
    ...
    def generate_api_documentation(self, output_path: str = None) -> str:
        ...
        try:
            ...
            self._save_generated_document(
                doc_id, 'API文档', 'api', template.template_id, content, output_path
            )
    ...
    def generate_system_documentation(self, output_path: str = None) -> str:
        ...
        try:
            ...
            self._save_generated_document(
                doc_id, '系统架构文档', 'system', template.template_id, content, output_path
            )
    ...
    def generate_user_manual(self, output_path: str = None) -> str:
        ...
        try:
            ...
            self._save_generated_document(
                doc_id, '用户手册', 'user_manual', template.template_id, content, output_path
            )
    ...
    def generate_deployment_guide(self, output_path: str = None) -> str:
        ...
        try:
            ...
            self._save_generated_document(
                doc_id, '部署指南', 'deployment', template.template_id, content, output_path
            )
            ...
            for decorator in node.decorator_list:
                if isinstance(decorator, ast.Call):
                    if hasattr(decorator.func, 'attr') and decorator.func.attr == 'route':
                        # 提取路径和方法
                        if decorator.args:
                            path = decorator.args[0].s if hasattr(decorator.args[0], 's') else str(decorator.args[0].value)
                            methods = ['GET']  # 默认方法
    ...
    def _save_generated_document(self, doc_id: str, doc_name: str, doc_type: str,
                               template_id: str, content: str, file_path: str = None):
        """保存生成的文档"""
        try:
            insert_sql = """
