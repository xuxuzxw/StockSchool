...
class FactorWeightEngine:
    ...
    def calculate_factor_importance(
        self, model_data: Dict[str, Any], X_test: pd.DataFrame, sample_size: int = 1000
    ) -> Dict[str, float]:
        """计算因子重要性
            ...
            if historical_weights:
                for factor in raw_weights:
                    if factor in historical_weights:
                        ...
                        raw_weights[factor] = (
                            raw_weights[factor] * (1 - decay_factor) + historical_weights[factor] * decay_factor
                        )
    ...
    def calculate_weight_stability(
        self, current_weights: Dict[str, float], historical_weights: Dict[str, float]
    ) -> Dict[str, float]:
        """计算权重稳定性
    ...
    def save_factor_weights(
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                conn.execute(
                    text(delete_sql),
                    {"model_name": model_name, "model_version": model_version, "calculation_date": calculation_date},
                )

                # 插入新记录
                for factor_name, weight in weights.items():
                    metadata = {"calculation_method": "shap_based", "constraints_applied": True}
    ...
    def get_factor_weights(
        self, model_name: str, model_version: str, calculation_date: Optional[str] = None
    ) -> Dict[str, FactorWeight]:
        """获取因子权重
    ...
    def get_weight_history(self, model_name: str, factor_name: Optional[str] = None, days: int = 30) -> pd.DataFrame:
        """获取权重变化历史
        ...
        if factor_name:
            ...
            """
            params = {"model_name": model_name, "factor_name": factor_name, "start_date": start_date}
        else:
            query_sql = """
            SELECT factor_name, old_weight, new_weight, change_reason,
                   changed_at, changed_by
            ...
            """
            params = {"model_name": model_name, "start_date": start_date}
        ...
        try:
            ...
            if rows:
                df = pd.DataFrame(
                    rows,
                    columns=["factor_name", "old_weight", "new_weight", "change_reason", "changed_at", "changed_by"],
                )
                return df
            else:
                return pd.DataFrame()
