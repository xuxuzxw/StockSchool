...
class DeploymentManager:
        ...
        try:
            ...
            default_configs = [
                DeploymentConfig(
                    ...
                    environment_variables={"FLASK_ENV": "development", "DEBUG": "true", "LOG_LEVEL": "DEBUG"},
                    resource_limits={"cpu": "1", "memory": "2g"},
                    health_check_config={"endpoint": "/health", "interval": 30, "timeout": 10, "retries": 3},
                    rollback_config={"auto_rollback": True, "rollback_threshold": 0.8, "rollback_timeout": 300},
                ),
                DeploymentConfig(
                    ...
                    redis_config={"host": "prod-redis.internal", "port": 6379, "database": 0},
                    environment_variables={"FLASK_ENV": "production", "DEBUG": "false", "LOG_LEVEL": "INFO"},
                    resource_limits={"cpu": "4", "memory": "8g", "replicas": 3},
                    health_check_config={"endpoint": "/health", "interval": 15, "timeout": 5, "retries": 5},
                    rollback_config={"auto_rollback": True, "rollback_threshold": 0.95, "rollback_timeout": 600},
                ),
            ]
    ...
    def deploy_application(self, config_id: str, version: str = None, created_by: str = None) -> str:
        """部署应用
    ...
    def rollback_application(self, config_id: str, target_version: str, created_by: str = None) -> str:
        """回滚应用
    ...
    def build_application(self, app_name: str, version: str, git_commit: str = None, changelog: str = None) -> str:
        """构建应用
    ...
    def get_deployment_history(
        self, app_name: str = None, environment: str = None, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """获取部署历史
    ...
    def generate_deployment_report(
        self, environment: str, start_date: datetime = None, end_date: datetime = None
    ) -> Dict[str, Any]:
        """生成部署报告
        ...
        try:
            ...
            # 获取部署统计
            deployment_stats = self._get_deployment_statistics(environment, start_date, end_date)

            # 获取健康检查统计
            health_stats = self._get_health_check_statistics(environment, start_date, end_date)
        ...
        try:
            ...
            for server_id in config.target_servers:
                ...
                for attempt in range(retries):
                    try:
                        response = requests.get(f"http://{server.ip_address}{endpoint}", timeout=timeout)
                        ...
                        task.logs.append(
                            f"服务器 {server.server_name} 健康检查异常 (尝试 {attempt + 1}/{retries}): {e}"
                        )
    ...
    def _generate_kubernetes_config(self, config: DeploymentConfig, version: str) -> Dict[str, Any]:
        ...
        return {
            "apiVersion": "apps/v1",
            "kind": "Deployment",
            "metadata": {"name": config.app_name, "namespace": config.environment},
            "spec": {
                "replicas": config.resource_limits.get("replicas", 1),
                "selector": {"matchLabels": {"app": config.app_name}},
                "template": {
                    "metadata": {"labels": {"app": config.app_name, "version": version}},
                    "spec": {
                        "containers": [
                            {
                                ...
                                "env": [
                                    {"name": key, "value": value} for key, value in config.environment_variables.items()
                                ],
                                "resources": {
                                    "limits": {
    ...
    def _update_server_config(self, conn: Connection, config: DeploymentConfig):
        ...
        try:
            # 更新环境变量
            env_content = "\n".join([f"export {key}={value}" for key, value in config.environment_variables.items()])
...
"""
    ...
    def _get_deployment_statistics(self, environment: str, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                result = conn.execute(
                    text(query_sql), {"environment": environment, "start_date": start_date, "end_date": end_date}
                )
                row = result.fetchone()
    ...
    def _get_health_check_statistics(
        self, environment: str, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        """获取健康检查统计"""
        try:
            query_sql = """
    ...
    def _get_server_status_summary(self, environment: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"total_servers": 0, "active_servers": 0, "inactive_servers": 0, "maintenance_servers": 0}
