...
class PredictionService:
    ...
    def _predict_single_stock(
        self, stock_code: str, model_data: Dict[str, Any], factor_data: pd.DataFrame, prediction_date: str
        ...
        try:
            ...
            if self.cache_enabled:
                try:
                    self.redis_client.setex(cache_key, 300, json.dumps(asdict(result), default=str))  # 5分钟缓存
                except Exception as e:
    ...
    def predict(self, request: PredictionRequest) -> BatchPredictionResult:
        ...
        try:
            ...
            for stock_code in request.stock_codes:
                ...
                if self.cache_enabled:
                    cache_key = self._get_cache_key(
                        stock_code, model_info["model_name"], model_info["model_version"], prediction_date
                    )
                    cached_result = self.redis_client.get(cache_key)
                    if cached_result:
                        try:
                ...
                if not cache_hit:
                    # 执行预测
                    prediction_result = self._predict_single_stock(stock_code, model_data, factor_data, prediction_date)
            ...
            if request.return_scores and predictions:
                predictions = self._add_scores_and_rankings(
                    predictions, model_info["model_name"], model_info["model_version"], prediction_date
                )
    ...
    def _add_scores_and_rankings(
        self, predictions: List[PredictionResult], model_name: str, model_version: str, prediction_date: str
    ) -> List[PredictionResult]:
        """添加评分和排名"""
        try:
            # 获取股票评分
            stock_codes = [p.stock_code for p in predictions]
            stock_scores = self.stock_scoring_engine.get_stock_scores(model_name, model_version, prediction_date)
    ...
    def get_prediction_history(self, stock_code: str, model_name: str, days: int = 30) -> pd.DataFrame:
        """获取预测历史
    ...
    def get_performance_stats(self, model_name: str, days: int = 7) -> Dict[str, Any]:
        """获取预测性能统计
