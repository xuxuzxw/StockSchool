...
class PermutationExplainer:
    ...
    def calculate_partial_dependence(self, X: pd.DataFrame, 
    ...
    def calculate_interaction_strength(self, X: pd.DataFrame, y: pd.Series,
        ...
        try:
            ...
            for idx1, idx2 in feature_pairs:
                interaction_strength = self._compute_pairwise_interaction(
                    X, y, idx1, idx2, n_samples
                )
    ...
    def _compute_pairwise_interaction(self, X: pd.DataFrame, y: pd.Series,
                                   idx1: int, idx2: int, n_samples: int) -> float:
        """
        计算成对特征交互强度
...
if __name__ == '__main__':
    ...
    # 测试特征重要性
    reg_importance = reg_explainer.calculate_feature_importance(
        X_test_reg, y_test_reg, n_repeats=5
    )
    ...
    # 测试特征重要性
    clf_importance = clf_explainer.calculate_feature_importance(
        X_test_clf, y_test_clf, n_repeats=5
    )
