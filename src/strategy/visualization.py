...
class ModelVisualizer:
    ...
    def plot_shap_summary(self, shap_values: np.ndarray, X: pd.DataFrame, max_display: int = 20) -> plt.Figure:
        """
        绘制SHAP摘要图
        ...
        try:
            ...
            # 创建SHAP Summary Plot
            shap.summary_plot(
                shap_values, X, feature_names=self.feature_names, max_display=max_display, show=False, plot_type="dot"
            )
    ...
    def plot_prediction_explanation(
        ...
        try:
            ...
            # 创建DataFrame
            expl_df = (
                pd.DataFrame({"feature": feature_names, "feature_value": feature_values, "shap_value": shap_values})
    ...
    def plot_feature_interactions(
        self, interaction_df: pd.DataFrame, top_n: int = 15, figsize: Tuple[int, int] = (12, 8)
    ) -> plt.Figure:
        """
        绘制特征交互强度图
        ...
        try:
            ...
            bars = ax.barh(y_pos, top_interactions["interaction_strength"], height=0.8, alpha=0.8, color="orange")
    ...
    def create_interactive_feature_importance(self, importance_df: pd.DataFrame, top_n: int = 20) -> go.Figure:
        """
        创建交互式特征重要性图
        ...
        try:
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    text=[
                        f"{val:.2f}<br>({shap:.3f})"
                        for val, shap in zip(expl_df["feature_value"], expl_df["shap_value"])
                    ],
                    textposition="outside",
                )
            )
            ...
            # 添加基准线
            fig.add_vline(
                x=base_value, line_dash="dash", line_color="black", annotation_text=f"基准值: {base_value:.3f}"
            )
    ...
    def plot_model_comparison(self, models_data: List[Dict[str, Any]], metric_name: str = "accuracy") -> plt.Figure:
        """
        绘制模型比较图
    ...
    def save_plot(self, fig: Union[plt.Figure, go.Figure], filename: str, format: str = "png", dpi: int = 300) -> str:
        """
        保存图形
        ...
        try:
            if isinstance(fig, plt.Figure):
                ...
                fig.savefig(full_filename, dpi=dpi, bbox_inches="tight", facecolor="white", edgecolor="none")
            else:
                # plotly图形
...
def quick_feature_importance_plot(
    importance_df: pd.DataFrame, top_n: int = 20, title: str = "特征重要性"
) -> plt.Figure:
    """
    快速绘制特征重要性图
...
def quick_shap_waterfall(explanation: Dict[str, Any]) -> go.Figure:
    ...
    fig.update_layout(title=f"SHAP瀑布图<br>预测值: {prediction:.3f}", height=600)
