...
from src.utils.gpu_utils import get_batch_size, get_device
...
class SHAPExplainer:
        ...
        try:
            ...
            if "tree" in model_name or "forest" in model_name or "gbm" in model_name:
                ...
                self.explainer = shap.TreeExplainer(
                    self.model, model_output="raw_values", feature_perturbation="tree_path_dependent"
                )
    ...
    def calculate_feature_importance(
        self, X: pd.DataFrame, y: pd.Series = None, background_samples: int = 100
    ) -> pd.DataFrame:
        """
        计算特征重要性
        ...
        try:
            ...
            # 创建结果DataFrame
            importance_df = (
                pd.DataFrame({"feature": self.feature_names, "importance": feature_importance})
