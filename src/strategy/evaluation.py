...
class StrategyEvaluator:
    ...
    def calculate_max_drawdown(self, returns: pd.Series) -> Dict[str, float]:
        ...
        max_dd_duration = (
            ...
            if hasattr(max_dd_end, "days")
            else len(cumulative[max_dd_start:max_dd_end])
        )
    ...
    def calculate_alpha(
        self, returns: pd.Series, benchmark_returns: pd.Series = None, periods_per_year: int = 252
    ) -> float:
        """
        计算阿尔法系数
            ...
            evaluation.update(
                {
                    ...
                    - self.calculate_annualized_return(benchmark_returns, periods_per_year),
                }
            )
    ...
    def generate_performance_report(
        self, returns: pd.Series, benchmark_returns: pd.Series = None, strategy_name: str = "Strategy"
    ) -> str:
        """
        生成策略表现报告
    ...
    def plot_performance(
        ...
        if rolling_window >= 10:
            ...
            ax4.text(0.5, 0.5, "数据不足\n无法计算滚动夏普比率", ha="center", va="center", transform=ax4.transAxes)
...
if __name__ == "__main__":
    ...
    # 模拟策略收益率
    strategy_returns = pd.Series(np.random.normal(0.001, 0.02, len(dates)), index=dates)  # 日均收益0.1%，波动率2%

    # 模拟基准收益率
    benchmark_returns = pd.Series(np.random.normal(0.0005, 0.015, len(dates)), index=dates)  # 日均收益0.05%，波动率1.5%
    ...
    # 生成报告
    report = evaluator.generate_performance_report(strategy_returns, benchmark_returns, "测试策略")
    print(f"\n{report}")
