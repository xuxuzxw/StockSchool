...
class BaseCollector:
    ...
    def _create_metric(self, component: str, status: str, **kwargs) -> HealthMetric:
        """创建健康指标"""
        return HealthMetric(component=component, status=status, **kwargs)
...
class SystemHealthCollector(BaseCollector):
    ...
    async def collect(self) -> List[HealthMetric]:
        ...
        try:
            ...
            metrics.append(
                self._create_metric(component="system", status="critical", message=f"系统监控失败: {str(e)}")
            )
    ...
    async def collect(self) -> List[HealthMetric]:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                response_threshold = (
                    config.get("monitoring_params", {}).get("alerts", {}).get("response_time_threshold", 1000)
                )
                ...
                metrics.append(
                    self._create_metric(component="database.connection", status="healthy", message="数据库连接正常")
                )
            ...
            metrics.append(
                self._create_metric(
                    component="database.connection", status="critical", message=f"数据库连接失败: {str(e)}"
                )
            )
        except Exception as e:
            self.logger.error(f"数据库健康检查失败: {e}")
            metrics.append(
                self._create_metric(component="database", status="critical", message=f"数据库监控失败: {str(e)}")
            )
...
class RedisHealthCollector(BaseCollector):
    ...
    async def collect(self) -> List[HealthMetric]:
        ...
        try:
            ...
            if pong:
                metrics.append(
                    self._create_metric(component="redis.connection", status="healthy", message="Redis连接正常")
                )
                ...
                metrics.append(
                    self._create_metric(component="redis.connection", status="critical", message="Redis ping失败")
                )

        except redis.ConnectionError as e:
            self.logger.error(f"Redis连接失败: {e}")
            metrics.append(
                self._create_metric(component="redis.connection", status="critical", message=f"Redis连接失败: {str(e)}")
            )
        except Exception as e:
            self.logger.error(f"Redis健康检查失败: {e}")
            metrics.append(
                self._create_metric(component="redis", status="critical", message=f"Redis监控失败: {str(e)}")
            )
    ...
    async def collect(self) -> List[HealthMetric]:
        ...
        try:
            ...
            if response.status_code == 200:
                metrics.append(self._create_metric(component="api.health", status="healthy", message="API服务正常"))
            ...
            metrics.append(
                self._create_metric(component="api.health", status="critical", message=f"API连接失败: {str(e)}")
            )
        except Exception as e:
            self.logger.error(f"API监控失败: {e}")
            metrics.append(self._create_metric(component="api", status="critical", message=f"API监控失败: {str(e)}"))
...
class CeleryHealthCollector(BaseCollector):
    ...
    async def collect(self) -> List[HealthMetric]:
        ...
        try:
            ...
            metrics.append(self._create_metric(component="celery.status", status="unknown", message="Celery监控未实现"))
        except Exception as e:
            self.logger.error(f"Celery监控失败: {e}")
            metrics.append(
                self._create_metric(component="celery", status="critical", message=f"Celery监控失败: {str(e)}")
            )
...
async def collect_system_health(config: Optional[CollectorConfig] = None) -> Dict[str, List[HealthMetric]]:
    ...
    for name, task in tasks:
        try:
            ...
            results[name] = [
                HealthMetric(component=f"{name}.error", status="critical", message=f"收集器执行失败: {str(e)}")
            ]
