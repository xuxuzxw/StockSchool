...
from .collectors import CollectorConfig, collect_system_health
from .config import get_monitoring_config
from .data_quality import DataQualityMonitor
from .data_quality_scheduler import DataQualityScheduler
from .notifications import NotificationManager
from .performance import SystemMonitor as PerformanceMonitor
...
class MonitoringSystem:
    ...
    def __init__(self):
        ...
        self.data_quality_scheduler: Optional[DataQualityScheduler] = None
        self._running = False
        self._tasks = []
    ...
    async def initialize(self):
        ...
        try:
            ...
            # 初始化收集器配置
            self.collector_config = Collectorget_config()
            ...
            # 初始化告警引擎
            self.alert_engine = AlertEngine(notification_manager=self.notification_manager)
            ...
            # 初始化数据质量调度器
            self.data_quality_scheduler = DataQualityScheduler()
    ...
    async def start(self):
        ...
        try:
            ...
            # 启动数据质量调度器（在单独线程中运行）
            if self.data_quality_scheduler:
                import threading

                scheduler_thread = threading.Thread(target=self.data_quality_scheduler.start, daemon=True)
            ...
            self._tasks = [
                ...
                asyncio.create_task(self._data_quality_monitoring_task()),
            ]
    ...
    async def stop(self):
        ...
        try:
            ...
            # 停止数据质量调度器
            if self.data_quality_scheduler:
                self.data_quality_scheduler.stop()
    ...
    async def get_data_quality_report(self, table_name: str):
        """获取数据质量报告"""
        if not self.data_quality_monitor:
            raise RuntimeError("数据质量监控器未初始化")

        return await self.data_quality_monitor.generate_quality_report(table_name)

    async def get_data_quality_history(self, table_name: str, days: int = 7):
        """获取数据质量历史"""
        if not self.data_quality_monitor:
            raise RuntimeError("数据质量监控器未初始化")

        return self.data_quality_monitor.get_quality_history(table_name, days)

    async def check_data_quality(self, table_name: str):
        """手动执行数据质量检查"""
        if not self.data_quality_monitor:
            raise RuntimeError("数据质量监控器未初始化")

        return await self.data_quality_monitor.check_data_quality(table_name)

    def get_data_quality_scheduler_status(self):
        """获取数据质量调度器状态"""
        if not self.data_quality_scheduler:
            raise RuntimeError("数据质量调度器未初始化")

        return self.data_quality_scheduler.get_status()
    ...
    async def _cleanup_task(self):
        ...
        while self._running:
            try:
                # 清理过期的性能数据
                if self.performance_monitor:
                    self.performance_monitor.cleanup_old_data(days=self.config.performance_retention_days)
    ...
    async def _data_quality_monitoring_task(self):
        ...
        while self._running:
            try:
                ...
                # 通过WebSocket推送数据质量状态
                if self.websocket_handler and quality_overview:
                    await self.websocket_handler.broadcast_data_quality_update(quality_overview)
                ...
                logger.error(f"数据质量监控任务出错: {e}")
                await asyncio.sleep(30)  # 出错时等待30秒再重试

    async def _get_quality_overview(self):
        """获取数据质量概览"""
        try:
            if not self.data_quality_scheduler:
                return None
            ...
            # 获取监控的表列表
            monitored_tables = self.data_quality_scheduler.monitored_tables
            ...
            # 获取每个表的最新质量状态
            for table_name in monitored_tables[:5]:  # 限制为前5个表，避免性能问题
                try:
                    history = self.data_quality_monitor.get_quality_history(table_name, days=1)
                    if history:
                        latest = history[0]
                        overview["table_status"][table_name] = {
                            "score": latest.get("overall_score", 0),
                            "status": (
                                "good"
                                if latest.get("overall_score", 0) >= 80
                                else "warning" if latest.get("overall_score", 0) >= 60 else "critical"
                            ),
                            "last_check": latest.get("timestamp"),
                        }
                    else:
                        overview["table_status"][table_name] = {"score": 0, "status": "unknown", "last_check": None}
                except Exception as e:
                    logger.debug(f"获取表 {table_name} 质量状态失败: {e}")
                    overview["table_status"][table_name] = {"score": 0, "status": "error", "last_check": None}
                ...
                if isinstance(metrics, dict) and "status" in metrics:
                    if metrics["status"] != "healthy":
                        ...
                        await self.alert_engine.trigger_alert(
                            ...
                            threshold="healthy",
                            context={"component": component, "metrics": metrics, "timestamp": metrics.get("timestamp")},
                        )
        except Exception as e:
