...
from src.monitoring.alerts import Alert, AlertEngine
from src.monitoring.collectors import CollectorConfig, collect_system_health
from src.monitoring.performance import SystemMonitor as PerformanceMonitor
...
class ConnectionManager:
        ...
        await self.send_personal_message(
            ...
            WebSocketMessage(
                ...
                data={
                    ...
                    "available_subscriptions": ["system_health", "alerts", "performance", "dashboard"],
                },
            ),
        )
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "subscription_stats": {
                subscription_type: sum(1 for subs in self.subscriptions.values() if subscription_type in subs)
...
class MonitoringWebSocketHandler:
    ...
    def setup(
        self, alert_engine: AlertEngine, performance_monitor: PerformanceMonitor, collector_config: CollectorConfig
    ):
        """设置依赖组件"""
            ...
            if message_type == "subscribe":
                ...
                if subscription_type:
                    ...
                    await self.manager.send_personal_message(
                        websocket,
                        WebSocketMessage(type="subscription_confirmed", data={"subscription_type": subscription_type}),
                    )
                ...
                if subscription_type:
                    ...
                    await self.manager.send_personal_message(
                        websocket,
                        WebSocketMessage(
                            type="unsubscription_confirmed", data={"subscription_type": subscription_type}
                        ),
                    )
                ...
                await self.manager.send_personal_message(
                    websocket, WebSocketMessage(type="pong", data={"server_time": datetime.now().isoformat()})
                )
                ...
                await self.manager.send_personal_message(
                    websocket, WebSocketMessage(type="error", data={"message": f"未知消息类型: {message_type}"})
                )

        except json.JSONDecodeError:
            await self.manager.send_personal_message(
                websocket, WebSocketMessage(type="error", data={"message": "无效的JSON格式"})
            )
        except Exception as e:
            logger.error(f"处理客户端消息失败: {e}")
            await self.manager.send_personal_message(
                websocket, WebSocketMessage(type="error", data={"message": f"处理消息失败: {str(e)}"})
            )
    ...
    async def send_current_data(self, websocket: WebSocket, data_type: str):
        ...
        try:
            if data_type == "system_health":
                ...
                await self.manager.send_personal_message(
                    websocket, WebSocketMessage(type="system_health_update", data=self.format_health_data(health_data))
                )
                ...
                if self.performance_monitor:
                    ...
                    await self.manager.send_personal_message(
                        websocket, WebSocketMessage(type="performance_update", data=performance_data)
                    )
                ...
                await self.manager.send_personal_message(
                    websocket, WebSocketMessage(type="dashboard_update", data=dashboard_data)
                )

        except Exception as e:
            logger.error(f"发送当前数据失败: {e}")
            await self.manager.send_personal_message(
                websocket, WebSocketMessage(type="error", data={"message": f"获取数据失败: {str(e)}"})
            )
            ...
            try:
                if self.collector_config:
                    ...
                    await self.manager.broadcast(
                        WebSocketMessage(type="system_health_update", data=self.format_health_data(health_data)),
        ...
        while self.is_running:
            try:
                if self.performance_monitor:
                    ...
                    if performance_data:
                        await self.manager.broadcast(
                            WebSocketMessage(type="performance_update", data=performance_data),
