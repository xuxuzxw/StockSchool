...
class SystemMonitor:
    ...
    def _collect_system_metrics(self):
        ...
        self._record_metric("disk_usage_percent", (disk_usage.used / disk_usage.total) * 100, "%", timestamp)
        ...
        try:
            ...
            self._record_metric("process_memory_mb", process.memory_info().rss / (1024**2), "MB", timestamp)
    ...
    def get_metric_summary(self, metric_name: str, window_minutes: int = None) -> Dict[str, float]:
        """获取指标摘要"""
...
class FunctionProfiler:
        ...
        def decorator(f):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    self._record_execution(function_name, module_name, execution_time, error_occurred)
    ...
    def _record_execution(self, function_name: str, module_name: str, execution_time: float, error_occurred: bool):
        """记录执行信息"""
        with self.lock:
...
class DatabaseProfiler:
    ...
    def _record_query(self, query_key: str, full_query: str, execution_time: float, error_occurred: bool):
        """记录查询统计"""
        with self.lock:
...
class PerformanceStorage:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_profiles_function ON function_profiles(function_name, module_name)"
            )
...
class PerformanceManager:
    ...
    def get_system_health(self) -> Dict[str, Any]:
        """获取系统健康状态"""
        health = {"timestamp": datetime.now().isoformat(), "status": "healthy", "issues": []}
...
if __name__ == "__main__":
    ...
    for profile in profiles:
        print(
            f"{profile.function_name}: 调用{profile.call_count}次, "
    ...
    for stat in query_stats[:5]:
        ...
        print(f"  执行{stat['count']}次, 总时间{stat['total_time']:.3f}s, " f"平均{stat['avg_time']*1000:.2f}ms")
