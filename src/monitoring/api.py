...
from src.monitoring.alerts import Alert, AlertEngine, AlertRule, AlertSeverity, AlertStatus, AlertType
from src.monitoring.collectors import CollectorConfig, HealthMetric, collect_system_health
...
def get_collector_config() -> CollectorConfig:
    ...
    if collector_config is None:
        collector_config = Collectorget_config()
...
async def get_system_health():
    ...
    try:
        ...
        return SystemHealthResponse(
            timestamp=datetime.now(), overall_status=overall_status, categories=categories, summary=summary
        )
...
async def get_alerts(status: Optional[str] = None, severity: Optional[str] = None, limit: int = 100):
    """获取告警列表"""
    try:
...
async def get_performance_metrics(
    metric_name: Optional[str] = None, window_seconds: Optional[int] = None, limit: int = 100
):
    """获取性能指标"""
    try:
