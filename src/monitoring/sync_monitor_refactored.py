...
from .decorators import cache_result, handle_exceptions, monitoring_operation
from .events import DatabaseEventObserver, EventPublisher, LoggingEventObserver, MetricsEventObserver
from .models import SyncEvent, SyncEventType, SyncTaskInfo, SyncTaskStatus
from .repositories import SyncEventRepository, SyncTaskRepository
...
class SyncMonitor:
    ...
    def update_task_progress(
        ...
        # 更新数据库
        success = self.task_repository.update_progress(task_id, progress, records_processed, records_failed)
    ...
    def complete_task_monitoring(
        ...
        if repo_success:
            ...
            event = SyncEvent(
                event_id="", event_type=event_type, task_id=task_id, timestamp=datetime.now(), data=event_data
            )
            self.event_publisher.publish(event)
...
def create_sync_monitor(config: Dict[str, Any]) -> SyncMonitor:
    ...
    # 创建监控器
    return SyncMonitor(task_repository=task_repository, event_repository=event_repository, constants=constants)
