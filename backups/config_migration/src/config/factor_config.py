...
class FactorConfig:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fundamental": {
                ...
                "profitability": {
                    "enabled": True,
                    "metrics": ["roe", "roa", "roic", "gross_margin", "net_margin"]
                },
                "valuation": {
                    "enabled": True,
                    "metrics": ["pe_ttm", "pb", "ps_ttm", "pcf_ttm", "ev_ebitda", "peg"]
                },
                "growth": {
                    "enabled": True,
                    "metrics": ["revenue_yoy", "net_profit_yoy"],
                    "periods": [1, 3, 5]  # 年数
                },
                "quality": {
                    "enabled": True,
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        technical_config = self.get_technical_config()
        if technical_config.get('enabled'):
            enabled_technical = sum(1 for category in ['momentum', 'trend', 'volatility', 'volume']
                                  if self.is_factor_enabled('technical', category))
            summary['enabled_technical_categories'] = enabled_technical
        
        fundamental_config = self.get_fundamental_config()
        if fundamental_config.get('enabled'):
            enabled_fundamental = sum(1 for category in ['profitability', 'valuation', 'growth', 'quality']
