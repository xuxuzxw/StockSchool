...
from .factor_models import FactorType, FactorResult, FactorValue
...
class MemoryCache:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            current_time = time.time()
            expired_count = sum(
                1 for expire_time in self.expire_times.values()
                if current_time > expire_time
            )
...
class RedisCache:
    ...
    def _serialize_value(self, value: Any) -> bytes:
        ...
        try:
            ...
            # 如果启用压缩且数据大于阈值
            if (self.config.use_compression and 
...
class FactorCache:
    ...
    def _generate_factor_key(self, ts_code: str, factor_name: str, 
    ...
    def _generate_result_key(self, ts_code: str, factor_type: str,
                           calculation_params: Dict[str, Any]) -> str:
        """生成计算结果缓存键"""
        # 创建参数的哈希值
    ...
    def get_factor_data(self, ts_code: str, factor_name: str,
                       start_date: Optional[date] = None,
                       end_date: Optional[date] = None) -> Optional[List[FactorValue]]:
        """获取因子数据（已重构为使用统一服务）"""
        try:
            from src.data.factor_data_service import FactorDataService, FactorQuery, DataSource
    ...
    def _legacy_get_factor_data(self, ts_code: str, factor_name: str,
                               start_date: Optional[date] = None,
                               end_date: Optional[date] = None) -> Optional[List[FactorValue]]:
        """旧版实现，用于向后兼容"""
    ...
    def set_factor_data(self, ts_code: str, factor_name: str,
    ...
    def get_calculation_result(self, ts_code: str, factor_type: str,
                             calculation_params: Dict[str, Any]) -> Optional[FactorResult]:
        """获取计算结果"""
    ...
    def set_calculation_result(self, ts_code: str, factor_type: str,
...
def cache_calculation_result(cache_ttl: int = 1800):
    ...
    def decorator(func):
        ...
        def wrapper(self, ts_code: str, *args, **kwargs):
            ...
            # 尝试从缓存获取
            cached_result = self.cache.get_calculation_result(
                ts_code, 'calculation', calculation_params
            )
            if cached_result is not None:
                return cached_result
            ...
            # 缓存结果
            if result is not None:
                self.cache.set_calculation_result(
                    ts_code, 'calculation', calculation_params, result, cache_ttl
                )
