...
> ```
> # 1. 创建主目录和所有子目录
> mkdir -p stockschool/src/{data,api,compute,strategy,utils,database,tests,monitoring,features,ai}
> ```
...
>    ```
>
> 2. **创建新文件 `stockschool/schema/financial/financial_reports.sql`**, 写入以下SQL代码，它定义了我们所有的表结构和索引:
>
>    SQL
>
>    ```
...
3. 手动同步代码 (备用):
   ...
   docker-compose up -d
   # 填写.env文件后执行
   psql "postgresql://stockschool:${DB_PASSWORD}@localhost:5432/stockschool" -f "stockschool/database_schema.sql"
   ```
>
...
> **任务**: 创建Celery任务队列配置。
>
> **注意**: Celery配置已整合到tasks.py中，无需单独创建celery_app.py, 写入以下代码:
>
> Python
>
> ```
...
> app.conf.update(
 ...
 enable_utc=True,
 task_routes={
 'src.compute.tasks.data_sync.*': {'queue': 'data_sync'},
 'src.compute.tasks.factor_calc.*': {'queue': 'factor_calc'},
 'src.compute.tasks.quality_check.*': {'queue': 'quality_check'}
 },
...
> **任务**: 实现特征商店的初步结构。
>
> **创建新文件 `stockschool/src/features/feature_store.py`**, 写入以下代码:
>
> Python
>
> ```
...
>
> ```
> **创建新文件 `stockschool/src/strategy/model_explainer.py`**, 写入以下代码:
> ```
...
> **任务**: 创建日志和告警系统。
>
> **创建新文件 `stockschool/src/monitoring/logger.py`**, 写入以下代码:
>
> Python
>
> ```
...
> ```
> return logger
> **创建新文件 `stockschool/src/monitoring/alerts.py`**, 写入以下代码:
> ```
...
> **任务**: 创建主入口文件和测试文件。
>
> **创建新文件 `stockschool/run.py` 和 `stockschool/src/api/main.py`**, 写入以下代码:
>
> Python
>
> ```
...
> 
>
> pytest stockschool/src/tests/ -k 'not test_technical'

> # 3. 运行主程序
>
> 
>
> python stockschool/run.py
>
> ```
