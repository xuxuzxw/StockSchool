...
审查代码: 请通读tushare_sync.py, akshare_sync.py和src/ai/training_pipeline.py等v1.1新增的文件。
...
python run.py --init-db && python -m cProfile -o profile.pstats src/compute/factor_engine.py
...
> test_db: image: timescale/timescaledb:latest-pg14 environment: - POSTGRES_USER=stockschool - POSTGRES_PASSWORD=test_password - POSTGRES_DB=test_stockschool ports:
  - "5433:5432"
volumes:
  - ./schema:/app/schema # 映射到本地的5433端口，避免与主数据库冲突
>
> ```
